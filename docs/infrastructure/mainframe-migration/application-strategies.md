...
- **重新裝載：** 您可以從大型主機移動現有的程式碼、程式和應用程式，然後重新編譯程式碼，以便在裝載于雲端實例的大型主機模擬器中執行。 此外，此方法通常會先將應用程式移至雲端式模擬器，然後再將資料庫移轉至雲端式資料庫。 在轉換資料和檔案時需要一些工程和重構。

    或者，您可以使用傳統的主機服務提供者來重新裝載。 使用雲端的主要好處之一，是可以將基礎結構管理工作外包。 您可以找到為您裝載大型主機工作負載的資料中心提供者。 此模式可換得時間、避免廠商鎖定，以及節省臨時成本。

- **淘汰：** 所有不再需要的應用程式都應該在遷移前淘汰。
...
- 如果應用程式支援多個執行緒，則具有較高密度個 vcpu 的 Vm 可利用多執行緒處理。
...
雲端的分散式架構依賴一組不同的開發工具，可提供新式實務和程式設計語言的優點。 為了簡化這項轉換，您可以將開發環境與其他專為模擬 IBM z/OS 環境而設計的工具搭配使用。 下列清單顯示 Microsoft 和其他廠商提供的選項：
...
- **應用程式** 要求者：接受來自應用程式的要求，再將其傳遞至應用程式伺服器。
- **線上資源介面卡：** 包含要在 CICS 交易中使用的應用程式要求者元件。
- **Batch 資源介面卡：** 針對 z/OS 批次應用程式，執行應用程式要求者元件。
- **互動式 SQL (ISQL) ：** 以 CICS 應用程式和介面的形式執行，讓使用者可以輸入 SQL 語句或操作員命令。
- **CICS 應用程式：** 使用 CICS 的可用資源和資料來源，以 CICS 的控制權來執行。
- **Batch 應用程式：** 執行處理邏輯，而不需要與使用者進行互動式通訊，例如，產生大量資料更新或從資料庫產生報表。
...
下圖顯示使用主要和次要站台的可能 Azure 部署。 在主要網站中，生產、預備和測試 Vm 會以高可用性部署。 次要站台供備份和災害復原之用。
