...
ms.locfileid: "83220357"
---
# <a name="deployment-acceleration-discipline-improvement"></a>部署加速專業領域改進

部署加速專業領域著重於建立原則，以確保資源會部署並以一致且可重複的方式進行設定，並且在其整個生命週期持續保有合規性。 在雲端治理的五個專業領域中，部署加速專業領域包含有關自動化部署、原始控制部署成品、監視已部署的資源以維護所需狀態，以及審核任何合規性問題的決策。
...
- 訓練開發小組和 IT 人員，以瞭解 DevSecOps 的原則和策略，以及在部署加速專業領域中完全自動化部署的重要性。
...
- 將所有秘密、密碼、憑證和連接字串儲存在[Azure Key Vault](https://docs.microsoft.com/azure/key-vault)中。
...
- 根據您組織的變更需求，自訂您的[部署加速工具鏈](./toolchain.md)。
- 將通知和報告自動化，以警示您潛在的設定問題或惡意威脅。
- 監視和報告應用程式與資源使用量。
- 報告部署後計量，並散發給專案關係人。
- 修改架構指導方針，以引導未來的採用程式。
- 繼續與您溝通，並定期訓練受影響的人員和小組，以確保持續遵循架構指導方針。
