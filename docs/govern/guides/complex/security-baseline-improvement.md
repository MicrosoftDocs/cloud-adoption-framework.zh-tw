...
CIO 已花了數個月的時間，與同事和公司的法務人員共同合作。 具有網路安全性專長的管理顧問致力於協助現有的 IT 安全性和 IT 治理小組草擬有關受保護資料的新原則。 該群組可以促進面板支援以取代現有的原則，讓已核准的雲端提供者能夠裝載敏感性個人和財務資料。 這需要採用一組安全性需求和一個治理流程，來驗證並記載對於那些原則的遵循情況。
...
12. 網路安全性小組會堅持廠商鎖定在單一雲端提供者平臺上產生加密金鑰的風險。 儘管此一說法尚未經過證實，但該小組目前接受此說法。
...
16. 可能會影響雲端部署的趨勢和入侵，應定期由安全性小組審查，以提供雲端中使用的安全性基準工具的更新。
...
21. 雲端治理流程必須包含身分識別基準小組的每季評論，以識別雲端資產設定應該避免的惡意執行者或使用模式。
...
2. 中樞和輪輻範本：
    ...
    3. 除了目前的參考架構之外，也應該建立網路安全性群組範本來捕捉任何埠封鎖或允許清單的需求，讓虛擬網路裝載防火牆。 此網路安全性群組與先前的群組不同，因為它會是第一個允許公用流量進入虛擬網路的網路安全性群組。
3. 建立 Azure 原則。 建立名為的原則 `hub NSG enforcement` ，以強制設定指派給在此訂用帳戶中建立之任何虛擬網路的網路安全性群組。 套用來賓設定的內建原則，如下所示：
...
1. 網路對等互連範本。 此範本會將每個訂用帳戶中的虛擬網路與公司 IT 訂用帳戶中的中樞虛擬網路對等互連。
    1. 先前章節的參考架構、[具有共用服務的中樞和輪輻拓撲](https://docs.microsoft.com/azure/architecture/reference-architectures/hybrid-networking/shared-services)，產生了可啟用虛擬網路對等互連的 Resource Manager 範本。
...
2. 修改網路安全性群組。 封鎖網路安全性群組中的所有公用**和**直接內部部署流量。 唯一的輸入流量應通過公司 IT 訂用帳戶中的虛擬網路對等互連。
    1. 在先前的反復專案中，已建立網路安全性群組來封鎖所有公用流量，並允許所有內部流量。 現在，我們想要將此網路安全性群組轉成一個位。
    2. 新的網路安全性群組設定應該會封鎖所有公用流量，以及來自本機資料中心的所有流量。
    3. 進入此虛擬網路的流量應該只來自虛擬網路對等另一端的虛擬網路。
3. Azure 資訊安全中心實作：
    1. 為包含受保護資料分類的任何管理群組設定 Azure 資訊安全中心。
    2. 依預設，將自動佈建設定為開啟，以確保修補的合規性。
    3. 建立 OS 安全性設定。 定義設定的 IT 安全性。
    4. 在初次使用 Azure 資訊安全中心時支援 IT 安全性。 將資訊安全中心的使用轉移到 IT 安全性，但保有治理持續改進的存取權。
...
6. Azure 藍圖：
    ...
    2. 將虛擬網路對等互連、網路安全性群組和 Azure 資訊安全中心範本新增至藍圖。
