...
1. 在此模型中，有三個訂用帳戶：**共用基礎結構**、**生產**和**開發**。 三個訂用帳戶都需要訂用帳戶擁有者，在範例中，您會為所有三個訂用帳戶使用相同的使用者帳戶。 **共用基礎結構**資源的管理方式類似上述前兩個範例，而第一個工作負載會與**生產**環境中的**app1-rg**資源群組和**開發**環境中相同名稱的資源群組相關聯。 **App1 工作負載擁有**者帳戶會新增至具有**參與者**角色的每個資源群組。
