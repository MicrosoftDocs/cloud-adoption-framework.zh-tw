...
ms.locfileid: "70818739"
---
# <a name="organize-your-azure-resources"></a>組織 Azure 資源

請使用下列功能和最佳做法來保護系統的重要資源。 標記資源，讓您可以依照對組織有意義的價值來追蹤資源。
...
Azure 中的資源組織結構有四個層級：管理群組、訂用帳戶、資源群組和資源。 下圖顯示這些層級的關聯性。

![管理階層關聯性的圖示](./media/organize-resources/scope-levels.png)

- **管理群組：** 管理群組是可協助您針對多個訂用帳戶管理存取、原則及合規性的容器。 管理群組內的所有訂用帳戶都會自動繼承套用到管理群組的條件。
...
在規劃合規性策略時，建議您與組織中擔任下列角色的人員合作：安全性和合規性、IT 管理、企業架構設計人員、網路、財務和採購。
...
良好的命名標準可協助您識別入口網站中、帳單上和指令碼中的資源。 您可能已經有內部部署基礎結構的命名標準。 將 Azure 新增至您的環境時，您應該將這些命名標準延伸至 Azure 資源。 命名標準可協助您更有效率地全面管理您的環境。 您可以使用 Azure 原則作為工具來對整個 Azure 環境強制執行命名標準。

::: zone target="docs"

我們建議您檢閱並採用[模式與實例指南](/azure/architecture/best-practices/naming-conventions)。
...
標記可快速找出資源和資源群組。 您可將標籤套用至 Azure 資源，以便以邏輯方式依照類別組織這些資源。 每個標記都是由一個名稱和一個值所組成。 例如，您可以將「環境」名稱和「生產」值套用至生產環境中的所有資源。

套用標記之後，您可以擷取訂用帳戶中具有該標記名稱和值的所有資源。 標記可讓您從不同的資源群組擷取相關資源，以協助您組織資源來達到計費或管理目的。
...
如需標記的進一步資訊，請參閱 [Azure 架構中心的 Azure 資源命名慣例](../../ready/considerations/name-and-tag.md#metadata-tags)
