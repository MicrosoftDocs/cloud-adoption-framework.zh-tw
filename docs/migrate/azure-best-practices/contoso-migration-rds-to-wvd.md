---
title: 將內部部署的遠端桌面服務移至 Azure Windows 虛擬桌面案例
...
ms.locfileid: "101791763"
---
<!-- docutune:casing Lakeside SysTrack -->
<!-- cSpell:ignore msiexec Logix Robocopy UPDs -->

# <a name="move-on-premises-remote-desktop-services-to-azure-windows-virtual-desktop-scenario"></a>將內部部署的遠端桌面服務移至 Azure Windows 虛擬桌面案例

Windows 虛擬桌面是在雲端中執行的全方位桌面與應用程式虛擬化服務。 它是唯一 (VDI) 的虛擬桌面基礎結構，可針對適用于企業的 Microsoft 365 應用程式提供簡化的管理、Windows 10 企業版多重會話優化，以及支援 (RDS) 環境的遠端桌面服務。 幾分鐘內就能在 Azure 上部署及調整 Windows 桌面和應用程式，並取得內建的安全性和合規性功能。
...
- **多會話 Windows 10 VDI：** 為 Contoso 使用者提供在雲端中虛擬化的唯一多會話 Windows 10 desktop，可高度擴充、最新且可在任何裝置上使用。
- 適用于 **企業的 Microsoft 365 應用程式優化：** 為企業經驗提供最佳的 Microsoft 365 應用程式，提供多會話的虛擬桌面案例，為 Contoso 的使用者提供最有效率的虛擬化體驗。
- **幾分鐘內即可部署及調整：** 透過 Azure 入口網站中的整合管理，在短短幾分鐘內快速將現代化和舊版桌面應用程式虛擬化及部署至雲端。
- **Azure 和 Microsoft 365 的安全和生產力：** 部署完整的智慧型解決方案，為每個人增強創意和協同作業。 轉移至 Microsoft 365 並取得 Office 365、Windows 10 及企業行動力 + 安全性。
...
Contoso 會使用 Azure 的規模調整、效能、安全性和創新，以充分利用現有的 Microsoft 365 授權。

其他優點可能包括：

- 從任何地方存取 Windows 虛擬桌面。
- 適用于企業環境的 Microsoft 365 應用程式優化。
...
RDS 會部署到內部部署資料中心。 Microsoft 365 已獲得授權，並在組織中使用。

### <a name="proposed-architecture"></a>建議的架構

- 同步處理 Active Directory 或 Azure Active Directory Domain Services。
...
| 考量 | 詳細資料 |
| --- | --- |
| **優點** | Windows 10 企業版多會話環境。 <br><br> 以雲端為基礎，允許從任何地方進行存取。 <br><br> 利用 Windows 虛擬桌面環境中的其他 Azure 服務，例如 Azure 檔案服務。 <br><br> 已針對 Microsoft 新式桌面進行優化。 |
| **缺點** | 為了充分優化 Azure，Contoso 必須重建針對多使用者會話優化的 Windows 10 映射。 <br><br> Windows 虛擬桌面不支援使用者設定檔磁片，因此必須將 Upd 遷移至 FSLogix 設定檔容器。 |

## <a name="migration-process"></a>移轉程序

Contoso 會使用 Lakeside 評定工具和 Azure 遷移，將 Vm 移至 Azure 中的 Windows 虛擬桌面。 Contoso 將需要：
...
1. 請確定網域服務（Active Directory 或 Azure Active Directory Domain Services）已與 Azure Active Directory 同步處理 (Azure AD) 。 確定可從 Azure 訂用帳戶和虛擬網路存取網域服務，以在其中部署 Windows 虛擬桌面。

    > [!NOTE]
    > 深入瞭解將內部部署 Active Directory 與 Azure AD 同步處理的 [AZURE Ad Connect](/azure/active-directory/hybrid/how-to-connect-install-express) 。

    <!-- -->

    > [!NOTE]
    > 瞭解如何布建 [Azure Active Directory Domain Services](/azure/active-directory-domain-services/tutorial-create-instance) ，以及如何將 azure AD 同步處理。

1. 建立新的 Azure 遷移專案。

   ![顯示建立新 Azure 遷移專案的螢幕擷取畫面。 ](./media/contoso-migration-rds-to-wvd/new-azure-migrate.png)
   *圖3：建立新的 Azure 遷移專案。*

1. 選取用來評估和遷移伺服器的選項、選取 [ **VDI**]，然後新增工具。

   ![顯示 VDI 的 Azure 遷移目標的螢幕擷取畫面。 ](./media/contoso-migration-rds-to-wvd/azure-migrate-goals-vdi.png)
   *圖4：目標為 Azure 遷移目標。*

1. 設定遷移作業資料的訂用帳戶、資源群組、專案名稱和地理位置。

   ![顯示將作業資料新增至 Azure 遷移專案的螢幕擷取畫面。 ](./media/contoso-migration-rds-to-wvd/add-a-tool.png)
   *圖5：將作業資料新增至遷移。*

    > [!IMPORTANT]
    > 此位置不是將部署新 Windows 虛擬桌面環境的位置。 只有與 Azure 遷移專案相關的資料會儲存在這裡。

1. 選取 **Lakeside： SysTrack** 作為評量工具。

1. 選取 **Azure 遷移：** 作為遷移工具的伺服器遷移。
...
1. 在 Lakeside 工具中選取 [ **使用 Azure 遷移註冊** ]，開始評量目前的環境。

   ![顯示使用 Azure 遷移 Lakeside 註冊的螢幕擷取畫面。 ](./media/contoso-migration-rds-to-wvd/lakeside-register-with-azure-migrate.png)
   *圖7：評定目前的環境。*

1. Contoso 會連接 Azure 遷移和 Lakeside，並接受任何要求的許可權。

   ![顯示登入以連接 Azure 和 Lakeside 的螢幕擷取畫面。 ](./media/contoso-migration-rds-to-wvd/lakeside-login.png)
   *圖8：將 Azure 連接到 Lakeside。*
...
Contoso 會使用 Azure 入口網站建立 Windows 虛擬桌面環境，以用於集區資源。 稍後會進行遷移步驟，以將個人桌面連接至相同的環境。
...
1. 指定訂用帳戶、資源群組和區域。 然後選取主機集區的名稱、桌上型電腦類型，以及預設的桌面使用者。 桌面類型設定為集區 **，因為 Contoso** 會為其使用者的新共用環境啟動。 預設桌面使用者可以保留空白。 繼續設定虛擬機器。
   ...
   *圖12：設定虛擬機器的必要條件。*
   ...
   - 因為 Contoso 正在建立共用伺服器，以使用新的 Windows 10 企業版多會話功能來進行虛擬機器設定，請將映射來源設定為資源 **庫**。 此選項可讓 Contoso 為 Vm 選取 Windows 10 企業版多會話映射。
   - 根據 Lakeside 評量使用者的角色，Contoso 會將使用者總數設為 **150**。
   - 其他設定包括磁片類型、Active Directory 網域加入 UPN 欄位、系統管理員密碼、新增機器的選用 OU 路徑、虛擬網路，以及用於新增伺服器的子網。
...
到目前為止，您已使用 Windows 10 企業版多會話的共用資源來進行遷移。 Contoso 可以開始將必要的應用程式部署到將使用 Windows 10 企業版多重會話的使用者。
...
Contoso 遷移程式的下一個步驟是將其持續性虛擬機器遷移至 Windows 虛擬桌面。 若要這樣做，Contoso 會回到在程式開始時所建立的 Azure 遷移：伺服器遷移作業。

1. Contoso 一開始先選取 [Azure 遷移：伺服器遷移工具] 中的 [ **探索** ]。

   ![顯示 [Azure 遷移：伺服器遷移探索] 選項的螢幕擷取畫面。 ](./media/contoso-migration-rds-to-wvd/wvd-persistent-discover.png)
   *圖15：探索伺服器遷移。*
...
1. 複寫提供者會下載、安裝並註冊至 Azure 遷移專案，以開始複寫至 Azure。
...
Contoso 安全性小組會檢查 Azure Vm 以判斷任何安全性問題。 若要控制存取權，小組會檢閱 VM 的網路安全性群組 (NSG)。 Nsg 是用來確保只有應用程式允許的流量可以到達它。 小組也會考慮使用 Azure 磁片加密和 Azure Key Vault 來保護磁片上的資料。
...
- Contoso 已有其 Vm 的現有授權，而且會利用適用于應用程式伺服器的 Azure 混合式權益。 Contoso 會轉換現有的 Azure VM，以便充分利用這個定價。
