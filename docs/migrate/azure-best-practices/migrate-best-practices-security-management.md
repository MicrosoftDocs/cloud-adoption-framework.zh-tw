...
在您針對移轉進行規劃及設計的同時，除了思考有關移轉本身的事項之外，您也需要考慮移轉後在 Azure 中的安全性和管理模型。 本文說明在遷移之後保護 Azure 部署的規劃和最佳作法。 其中也涵蓋了進行中的工作，讓您的部署以最佳層級執行。
...
Azure 租使用者系統管理員必須啟用安全性功能，以保護工作負載免于遭受攻擊。 Azure 資訊安全中心能提供統一的安全性管理。 從資訊安全中心，您可以跨工作負載套用安全性原則、限制威脅暴露，以及偵測和回應攻擊。 資訊安全中心會分析整個 Azure 租使用者的資源和設定，並提出安全性建議，包括：

- **集中式原則管理：** 藉由集中管理所有混合式雲端工作負載的安全性原則，確保符合公司或法規的安全性需求。
- **持續安全性評估：** 監視機器、網路、儲存體和資料服務以及應用程式的安全性狀態，以找出潛在的安全性問題。
- 可**操作的建議：** 在攻擊者利用優先順序和可操作的安全性建議之前，先修復安全性弱點。
- **排定優先順序的警示和事件：** 先專注于最重要的威脅，並加上優先順序的安全性警示和事件。

除了評量和建議之外，Azure 資訊安全中心還提供您可以針對特定資源啟用的其他安全性功能。

- **及時（JIT）存取。** 使用 Azure Vm 上管理埠的即時控制存取，減少您的網路攻擊面。
  - 在網際網路上開啟 VM RDP 埠3389，會將 Vm 公開給不良執行者的持續活動。 由於 Azure IP 位址是眾所周知的，因此駭客會持續對它們進行探查，以對開啟的 3389 連接埠發動攻擊。
  - 「即時」會使用網路安全性群組（Nsg），以及限制特定埠開啟時間量的傳入規則。
  - 啟用即時存取後，資訊安全中心會檢查使用者是否具有 VM 的角色型存取控制（RBAC）寫入存取權限。 此外，您可以指定使用者如何連接至 Vm 的規則。 如果許可權正常，則會核准存取要求，而資訊安全中心會將 Nsg 設定為允許輸入流量進入選取的埠，以達您指定的時間量。 當時間到期時，Nsg 會回到其先前的狀態。
- **適應性應用程式控制。** 藉由使用動態允許清單，藉由控制要在其上執行的應用程式，將軟體和惡意程式碼保留在 Vm 上。
...
- 深入瞭解[即時 VM 存取](https://docs.microsoft.com/azure/security-center/security-center-just-in-time)。
...
### <a name="encryption-for-infrastructure-as-a-service-iaas"></a>基礎結構即服務（IaaS）的加密

- **虛擬機器：** 針對 Vm，您可以使用 Azure 磁碟加密來加密您的 Windows 和 Linux IaaS VM 磁片。
  - Azure 磁碟加密使用適用于 Windows 的 BitLocker 和適用于 Linux 的 dm crypt，為作業系統和資料磁片提供磁片區加密。
...
### <a name="encryption-for-platform-as-a-service-paas"></a>平臺即服務（PaaS）的加密

不同于 IaaS，您在其中管理自己的 Vm 和基礎結構，在 PaaS 模型平臺中，而基礎結構是由提供者所管理。 您可以專注于核心應用程式邏輯和功能。 由於 PaaS 服務類型眾多，因此基於安全性目的，我們會對每個服務進行個別評估。 例如，讓我們看看您可以如何為 Azure SQL Database 啟用加密。
...
特別的是，較舊的 Azure 遷移 Vm 可能不會安裝適當的反惡意程式碼層級。 Azure 提供免費的端點解決方案，可協助保護 VM 不受病毒、間諜軟體及其他惡意程式碼的威脅。

- Azure 雲端服務和虛擬機器的 Microsoft Antimalware 會在已知的惡意或垃圾軟體嘗試自行安裝時產生警示。
- 它是在背景執行的單一代理程式解決方案，不需要人為介入。
- 在 Azure 資訊安全中心中，您可以輕鬆地識別沒有執行 endpoint protection 的 Vm，並視需要安裝 Microsoft 反惡意程式碼。

  ![Vm 反惡意程式碼的螢幕擷取畫面。 ](./media/migrate-best-practices-security-management/antimalware.png)
  _圖1： vm 的反惡意程式碼。_

**瞭解更多資訊：**

- 瞭解[適用于 Azure 的 Microsoft Antimalware](https://docs.microsoft.com/azure/security/fundamentals/antimalware)。
...
- **Azure Key Vault：** 現今，web 應用程式開發人員正在採取步驟，以確保機密資訊不會洩漏這些檔案。 保護資訊的其中一個方法，是將它從檔案中擷取出來，並置於 Azure Key Vault 中。
  ...
  - Azure Key Vault 可讓您透過 Azure 安全性控制來鎖定存取，並順暢地執行輪流金鑰。 Microsoft 不會看到或解壓縮您的資料。
- **適用于 Power Apps 的 App Service 環境：** 如果您遷移的應用程式需要額外的保護，請考慮新增 App Service 環境和 Web 應用程式防火牆來保護應用程式資源。
  - App Service 環境提供完全隔離且專用的環境來執行應用程式，例如 Windows 和 Linux web 應用程式、Docker 容器、行動應用程式和函式應用程式。
  - 對於非常大規模、需要隔離和安全網路存取，或具有高記憶體使用率的應用程式而言，這非常有用。
- **Web 應用程式防火牆：** 這是 Azure 應用程式閘道的功能，可提供 web 應用程式的集中式保護。
  - 它能在無需後端程式碼修改的情況下保護 Web 應用程式。
  - 它會在應用程式閘道之後，同時保護多個 web 應用程式。
  - 您可以使用 Azure 監視器來監視 Web 應用程式防火牆。 Web 應用程式防火牆已整合到 Azure 資訊安全中心中。

  ![Azure Key Vault 和保護 web 應用程式的圖表。 ](./media/migrate-best-practices-security-management/web-apps.png)
  _圖2： Azure Key Vault。_
...
RBAC 可以將角色指派給安全性主體，例如「擁有者」、「參與者」和「讀者」，以及定義角色可執行之作業的角色定義（許可權集合）。 RBAC 也可以設定範圍以設定角色的界線。 範圍可以設定於數個層級上，包括管理群組、訂用帳戶、資源群組或資源。
...
- [瞭解](https://docs.microsoft.com/azure/role-based-access-control/role-assignments-portal)如何使用 RBAC 和 Azure 入口網站來管理存取權。
...
- 安全性報告的存取方式取決於您的 Azure AD 授權。 使用免費和基本授權時，您會取得有風險的使用者和登入清單。使用 premium 授權時，您會取得基礎事件資訊。
- 您可以將活動記錄路由傳送到各種端點，以進行長期保留並取得資料見解。
- 請務必檢查記錄，或整合您的安全性資訊和事件管理（SIEM）工具，以自動審查異常狀況。 如果您不是使用 premium 授權，則需要自行進行大量分析，或使用您的 SIEM 系統。 分析包含尋找具風險的登入和事件，以及其他使用者攻擊模式。

  ![Azure AD 使用者和群組的螢幕擷取畫面。 ](./media/migrate-best-practices-security-management/azure-ad.png)
  _圖4： Azure AD 使用者和群組。_
...
Azure 提供能提供進階安全性選項的其他安全性功能。 請注意，下列一些最佳作法需要附加元件授權和 premium 選項。

- **執行 Azure AD 管理單位（AU）。** 使用基本的 Azure 存取控制來委派系統管理工作以支援人員，可能會是一件相當困難的事。 給予支援人員存取權以管理 Azure AD 中的所有群組，對組織的安全性而言可能不是理想的方法。 使用 AU 可讓您以和內部部署組織單位 (OU) 類似的方式，將 Azure 資源隔離在容器內。 若要使用 AU，AU 管理員必須具有 premium Azure AD 授權。 如需詳細資訊，請參閱[Azure Active Directory 中的系統管理單位管理](https://docs.microsoft.com/azure/active-directory/users-groups-roles/directory-administrative-units)。
- **使用多重要素驗證。** 如果您有進階 Azure AD 授權，您可以在系統管理員帳戶上啟用並強制執行多重要素驗證。 網路釣魚是用來入侵帳戶認證的最常見方式。 當不正確的執行者具有系統管理員帳號憑證時，就不會將它們從最接近的動作中停止，例如刪除所有的資源群組。 您可以用數種方式建立多重要素驗證，包括電子郵件、驗證器應用程式和電話簡訊。 身為系統管理員，您可以選取最不具侵入性的選項。 多重要素驗證會與威脅分析和條件式存取原則整合，以隨機要求多重要素驗證挑戰回應。 深入了解[安全性指引](https://docs.microsoft.com/azure/active-directory/authentication/multi-factor-authentication-security-best-practices)，以及[如何設定多重要素驗證](https://docs.microsoft.com/azure/active-directory/authentication/multi-factor-authentication-security-best-practices)。
- **實作條件式存取。** 在大部分中小型組織中，Azure 系統管理員和支援小組可能位於單一地理位置。 在此情況下，大部分的登入都來自相同的區域。 如果這些位置的 IP 位址是相當靜態的，則您應該不會看到系統管理員從這些區域之外登入。 即使遠端不良動作專案會危害系統管理員的認證，您也可以執行像是條件式存取（結合多重要素驗證）的安全性功能，以防止從遠端位置登入。 這也可以防止來自隨機 IP 位址的偽造位置。 若要深入瞭解[條件式存取](https://docs.microsoft.com/azure/active-directory/conditional-access/overview)，請參閱 Azure AD 中的條件式存取的[最佳做法](https://docs.microsoft.com/azure/active-directory/conditional-access/best-practices)。
- **審查企業應用程式許可權。** 經過一段時間，系統管理員就會選取 Microsoft 和協力廠商連結，而不需要知道其對組織的影響。 連結可以呈現同意畫面，以將許可權指派給 Azure 應用程式。 這可能會允許存取讀取 Azure AD 資料，或甚至是完整存取權來管理整個 Azure 訂用帳戶。 您應該定期檢查您的系統管理員和使用者允許存取 Azure 資源的應用程式。 請確定這些應用程式只有必要的許可權。 此外，每季或半年您可以透過電子郵件將使用者連結至應用程式頁面，讓他們知道他們允許存取其組織資料的應用程式。 如需詳細資訊，請參閱[我的應用程式清單中的非預期應用程式](https://docs.microsoft.com/azure/active-directory/manage-apps/application-types)和[如何控制](https://docs.microsoft.com/azure/active-directory/manage-apps/remove-user-or-group-access-portal)Azure AD 中的應用程式指派。

## <a name="managed-migrated-workloads"></a>管理已移轉的工作負載

在下列各節中，我們將建議 Azure 管理的一些最佳作法，包括：
...
請確定您的資源群組具有有意義的名稱，讓系統管理員和支援小組成員可以輕鬆地辨識和掃描。 這可以大幅提升生產力和效率。
...
您最不想遇到的情況，便是資源群組被意外刪除而消失不見。 我們建議您執行刪除鎖定，這樣就不會發生這種情況。
...
通常，僅使用與資源相關的資源組名，並不會提供足夠的中繼資料來進行機制的有效執行，例如在訂用帳戶內進行內部計費或管理。

- 最佳做法是使用 Azure 標記來新增有用的中繼資料，以便在上進行查詢和報告。
- 標記能提供搭配您所定義的屬性，以邏輯方式組織資源的方式。 標記可以直接套用到資源群組或資源。
- 標記可以套用到資源群組或是個別的資源之上。 資源群組標記不會被群組中的資源所繼承。
- 您可以使用 PowerShell 或 Azure 自動化，或標記個別群組和資源，將標記自動化。
...
就像藍圖可讓工程師和架構師草擬專案的設計參數，Azure 藍圖服務可讓雲端架構設計人員和中央 IT 小組定義一組可重複使用的 Azure 資源。 這可協助他們執行並遵循組織的標準、模式和需求。 使用 Azure 藍圖，開發小組可以快速建立並建立符合組織合規性需求的新環境。 這些新環境具有一組內建元件（例如網路功能），可加速開發和傳遞。

- 使用藍圖來協調資源群組、Azure Resource Manager 範本，以及原則和角色指派的部署。
- 在全域散發的服務中儲存藍圖，Azure Cosmos DB。 藍圖物件會複寫至多個 Azure 區域。 無論藍圖部署資源的區域為何，複寫都會提供藍圖的低延遲、高可用性和一致的存取權。
...
參考架構會依案例編排。 其中包含有關管理、可用性、擴充性和安全性的最佳做法和建議。 App Service 環境提供完全隔離且專用的環境來執行應用程式，例如 Windows 和 Linux web 應用程式、Docker 容器、行動應用程式和功能。 App Service 能將 Azure 的功能新增到您的應用程式，其中包括安全性、負載平衡、自動調整和自動化管理。 您也可以利用它的 DevOps 功能，例如來自 Azure DevOps 和 GitHub 的持續部署、套件管理、預備環境、自訂網域和 SSL 憑證。 App Service 適用于需要隔離和安全網路存取的應用程式，以及使用大量記憶體和其他需要調整之資源的應用程式。
...
如果您的組織具有多個訂用帳戶，您便需要為它們管理存取、原則及合規性。 Azure 管理群組可以在訂用帳戶之上提供範圍層級。 以下是一些秘訣：

- 您會將訂用帳戶整理到稱為管理群組的容器中，並將治理條件套用至它們。
- 管理群組中的所有訂用帳戶都會自動繼承管理群組條件。
- 無論您擁有哪種類型的訂用帳戶，管理群組都可提供大規模的企業級管理。
...
Azure 原則是一項服務，您可用來建立、指派和管理原則。 原則會對您的資源強制執行不同的規則和效果，讓這些資源保持符合您的公司標準和服務等級協定。

Azure 原則會評估您的資源，掃描那些不符合您原則規範的資源。 例如，您可以建立原則，只允許您環境中 Vm 的特定 SKU 大小。 當您建立和更新資源，以及掃描現有資源時，Azure 原則將會評估此設定。 請注意，Azure 提供一些可供您指派的內建原則，您也可以自行建立。
...
規劃商務持續性和嚴重損壞修復（BCDR）是一項重要的練習，您應該在 Azure 遷移規劃過程中完成此作業。 根據法律規定，您的合約可能會包含*強制不可抗力*子句，其因應較大的力（例如颶風或地震）而造成託辭責任。 但您也有責任確保服務會繼續執行，並在必要時進行復原（發生嚴重損壞時）。 您達成此義務的能力，可能會左右您公司的未來發展。
...
當遷移至 Azure 時，請瞭解雖然 Azure 平臺會提供一些內建的復原功能，但您必須設計 Azure 部署以利用它們。

- 您的 BCDR 解決方案將取決於您的公司目標，並會受到您的 Azure 部署策略影響。 基礎結構即服務 (IaaS) 和平台即服務 (PaaS) 部署會為 BCDR 帶來不同的挑戰。
- 備妥之後，您的 BCDR 解決方案應該定期測試，以檢查您的策略是否仍然可行。

### <a name="back-up-an-iaas-deployment"></a>備份 IaaS 部署

在大部分的情況下，內部部署工作負載會在移轉後淘汰，且您必須對用於備份資料的內部部署策略進行擴充或加以取代。 如果您將整個資料中心遷移至 Azure，您將需要使用 Azure 技術或協力廠商整合式解決方案來設計和實施完整的備份解決方案。
...
Azure 備份會建立儲存在 Azure 儲存體中的資料復原點。 Azure 備份可以備份 Azure VM 磁碟，以及 Azure 檔案 (預覽)。 Azure 檔案儲存體提供雲端中的檔案共用，可透過伺服器訊息區來存取。
...
- **復原服務保存庫中的直接備份。** 您可以部署 Azure 備份復原服務保存庫，來備份您的 IaaS VM。 這會提供單一位置來追蹤和管理備份，以及細微的備份和還原選項。 備份會在檔案和資料夾層級，每天最多三次。 它不是應用程式感知，而且不支援 Linux。 使用此方法，在您想要備份的每個 VM 上安裝 Microsoft Azure 復原服務（MARS）代理程式。
...
基於安全性，Azure 備份會使用 AES-256 來加密資料。 它會透過 HTTPS 將它傳送至 Azure。 在 Azure 中備份的待用資料會使用[Azure 儲存體加密](https://docs.microsoft.com/azure/storage/common/storage-service-encryption)進行加密。
...
您可以複製整個 Blob，或使用增量快照複製以僅複製差異變更，並減少儲存空間。 為求額外的預防措施，您可以啟用 Blob 儲存體帳戶的虛刪除。 啟用這項功能後，已刪除的 blob 會標示為刪除，但不會立即清除。 在過渡期間，您可以還原 blob。
...
除了保護資料以外，BCDR 規劃也必須考慮如何讓應用程式和工作負載在發生嚴重損壞時保持可用。 針對在 Azure IaaS Vm 和 Azure 儲存體上執行的工作負載，請考慮下列各節中的解決方案。

#### <a name="azure-site-recovery"></a>Azure Site Recovery

Azure Site Recovery 是主要的 Azure 服務，可確保 Azure Vm 可以上線，並在發生中斷時讓 VM 應用程式可供使用。

Site Recovery 會將 Vm 從主資料庫複寫到次要 Azure 區域。 如果發生嚴重損壞，您會從主要區域將 Vm 故障，並在次要區域中繼續正常地存取它們。 當作業返回正常時，您便可以將 VM 容錯回復到主要區域。

  ![Azure Site Recovery 的圖表。](./media/migrate-best-practices-security-management/site-recovery.png)
  _圖11： Site Recovery。_
...
您將工作負載移轉到 Azure 的原因，可能是為了它強大的擴充能力。 但移動您的工作負載並不表示 Azure 會自動執行調整，而不需要您輸入。 以下是兩個範例：

- 如果您的行銷組織推播新的電視公告，以提高300% 的流量，這可能會導致網站可用性問題。 您剛遷移的工作負載可能會達到指派的限制，並損毀。
- 如果您已遷移的工作負載發生分散式阻斷服務（DDoS）攻擊，在此情況下，您不會想要進行調整。 您想要防止攻擊來源到達您的資源。

這兩種情況有不同的解決方式，但您需要深入瞭解使用方式和效能監視的情況。

- Azure 監視器可以協助呈現這些計量，並提供警示、自動調整、事件中樞和邏輯應用程式的回應。
- 您也可以整合協力廠商 SIEM 應用程式來監視 Azure 記錄，以取得審核和效能事件。
...
- 當您的監視資料中發現條件時，警示會主動通知您。 您接著便可以在系統使用者注意到這些問題之前解決他們。 您可以對計量值、記錄搜尋查詢、活動記錄事件、平臺健全狀況和網站可用性發出警示。
...
搭配最新的作業系統和軟體更新來將 Azure VM 保持在最新狀態，是一件規模龐大的工作。 能夠呈現所有 Vm、找出所需的更新，以及自動推送那些更新，非常有價值。

- 您可以使用 Azure 自動化中的更新管理來管理作業系統更新。 這適用于執行部署于 Azure、內部部署及其他雲端提供者之 Windows 和 Linux 電腦的機器。
- 使用「更新管理」來快速評估所有代理程式電腦上可用更新的狀態，以及管理更新安裝。
- 您可以直接從 Azure 自動化帳戶啟用 Vm 的更新管理。 您也可以在 Azure 入口網站中從 VM 頁面更新單一 VM。
- 此外，您可以使用 System Center Configuration Manager 來註冊 Azure Vm。 然後，您可以將 Configuration Manager 工作負載遷移至 Azure，並從單一 web 介面執行報告和軟體更新。
...
- 強制執行變更管理程式時，您可以使用 audit 記錄，將 Azure 變更記錄連結至現有的變更要求。 然後，如果您在沒有對應變更要求的情況下看到所做的變更，您可以調查程式中發生了什麼問題。
...
- 邏輯會套用至接收的資料，而雲端服務則會記錄資料。
- 在 [變更追蹤] 儀表板上，您可以輕鬆地查看伺服器基礎結構中所做的變更。

  ![變更管理的螢幕擷取畫面。 ](./media/migrate-best-practices-security-management/change.png)
  _圖19：變更管理。_
