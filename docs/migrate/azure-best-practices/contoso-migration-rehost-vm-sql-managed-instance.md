...
description: 瞭解 Contoso 如何使用 Azure SQL 受控執行個體，在 Azure Vm 上如何內部部署應用程式。
author: givenscj
ms.author: abuck
ms.date: 07/01/2020
ms.topic: conceptual
...
本文說明虛構公司 Contoso 如何使用 Azure Migrate，將在 VMware 虛擬機器上執行的兩層式 Windows .NET 前端應用程式遷移 (Vm) 至 Azure VM。 它也會說明 Contoso 如何將應用程式資料庫移轉至 Azure SQL 受控執行個體。
...
- Contoso 不想要使用此應用程式 Azure SQL Database。 且正在尋求替代方案。
...
Contoso 在解決方案設計過程中進行了 Azure SQL Database 與 SQL 受控執行個體之間的功能比較。 下列考慮有助於公司決定使用 SQL 受控執行個體。

- SQL 受控執行個體的目標是要提供與最新內部部署 SQL Server 版本幾乎100% 的相容性。 針對在內部部署環境或基礎結構即服務 (IaaS) Vm 上執行 SQL Server 的客戶，我們建議使用 SQL 受控執行個體，並想要將其應用程式遷移至完全受控的服務，並將其設計變更降至完全受控。
- Contoso 打算將大量的應用程式從內部部署遷移至 IaaS。 其中有許多應用程式都是 ISV 提供的。 Contoso 發現，使用 SQL 受控執行個體將有助於確保這些應用程式的資料庫相容性，而不是使用可能不支援的 SQL Database。
...
- 使用軟體保證，Contoso 可以使用適用于 SQL Server 的 Azure Hybrid Benefit，在 SQL 受控實例上交換其現有授權的折扣費率。 基於這個理由，Contoso 最多可省下30% 的 SQL 受控執行個體費用。
- SQL 受控執行個體完全包含在虛擬網路中，因此可為 Contoso 的資料提供更高的隔離和安全性。 Contoso 可以取得公用雲端的優點，同時讓環境與公用網際網路隔離。
- SQL 受控執行個體支援許多安全性功能。 其中包含 Always Encrypted、動態資料遮罩、資料列層級安全性和威脅偵測。
...
1. Web 層將會使用 Azure Migrate，藉由使用隨即轉移進行遷移。 此程序需要準備內部部署 VMware 環境、設定和啟用複寫，以及將 VM 容錯移轉至 Azure 以便遷移。
...
> - **步驟2：準備 Azure 資料庫移轉服務。** Contoso 必須註冊資料庫移轉提供者、建立執行個體，然後建立資料庫移轉服務專案。 Contoso 也必須將共用存取簽章 (SAS) 統一資源識別項 (URI) 用於資料庫移轉服務實例。 SAS URI 可提供 Contoso 儲存體帳戶中資源的委派存取權，讓 Contoso 可以將有限的許可權授與給儲存體物件。 Contoso 會設定 SAS URI，讓 Azure 資料庫移轉服務可以存取服務上傳 SQL Server 備份檔案的儲存體帳戶容器。
> - **步驟3：針對 Azure Migrate：伺服器遷移工具準備 Azure。** Contoso 會將伺服器遷移工具新增至其 Azure Migrate 專案。
> - **步驟4：準備內部部署 VMware 以進行 Azure Migrate：伺服器遷移。** Contoso 會準備帳戶以進行 VM 探索，並在遷移後準備連接至 Azure Vm。
> - **步驟5：複寫內部部署 Vm。** Contoso 會設定複寫，並開始將 Vm 複寫至 Azure 儲存體。
> - **步驟6：透過 Azure 資料庫移轉服務遷移資料庫。** Contoso 遷移資料庫。
> - **步驟7：使用 Azure Migrate：伺服器遷移來遷移 Vm。** Contoso 會執行測試遷移，以確定一切都正常運作，然後執行完整遷移以將 VM 移至 Azure。
...
- 授與資料庫移轉服務存取 Azure 儲存體的許可權，以便上傳用來遷移資料庫的備份檔案。 若要提供 Azure 儲存體的存取權，請建立 Azure Blob 儲存體容器。 產生 Blob 儲存體容器的 SAS URI。
- 建立 Azure 資料庫移轉服務專案。
...
1. 在 Azure Migrate 專案中，他們會移至 [**伺服器**  >  **Azure Migrate：伺服器遷移**]。 然後 **選取 [** 複寫]。
...
1. 系統管理員會建立資料庫移轉服務專案。 他們會選取 **SQL Server** 來源伺服器類型和 **Azure SQL 受控執行個體** 作為目標。
...
1. 在 [**遷移目標**  >  **伺服器**  >  **Azure Migrate：伺服器遷移**] 中，他們會選取 [**測試遷移的伺服器**]。
...
在遷移程式的最後一個步驟中，Contoso 管理員會將應用程式的連接字串更新為指向在 Contoso 的 SQL 受控實例上執行的遷移後資料庫。
...
Contoso 安全性小組會檢查 Azure Vm 和 SQL 受控實例，以檢查其實行是否有任何安全性問題：
...
在本文中，Contoso 會使用 Azure Migrate 將應用程式前端 VM 遷移至 Azure，以在 Azure 中如何 SmartHotel360 應用程式。 Contoso 會使用 Azure 資料庫移轉服務，將內部部署資料庫移轉至 SQL 受控實例。