...
3. Web 層將使用 Site Recovery 的隨即轉移來進行遷移。 此程序需要準備內部部署 VMware 環境、設定和啟用複寫，以及將 VM 容錯移轉至 Azure 以便遷移。
...
6. 此工具會檢查虛擬機器與 Azure 的連線。 建立連線之後，他們會選取 [登入] 以登入 Azure 訂用帳戶。 認證必須能夠存取在其中註冊組態伺服器的保存庫。
...
2. 他們使用預設設定：
    ...
    - **應用程式一致快照集頻率：** 預設值為1小時。 此值會指定應用程式一致快照的建立頻率。
