...
本節中的最佳作法可協助您估計成本、針對虛擬機器執行適當的大小調整， (Vm) 和儲存體、使用 Azure Hybrid Benefit、使用 Azure 保留的 VM 執行個體，以及評估訂用帳戶之間的雲端費用。
...
- **Azure 定價計算機：** 選取您想要預估的產品，例如 Vm 和儲存體，然後將成本輸入至計算機以建立預估。

 ![Azure 定價計算機 ](./media/migrate-best-practices-costs/pricing.png) _azure 定價計算機。_
...
| **計算最佳化** | CPU 對記憶體的比例高。 | 適用于中量流量 web 伺服器、網路設備、批次處理、應用程式伺服器。 |
| **記憶體優化** | 高記憶體對 CPU。 | 適用於關聯式資料庫、中型到大型快取、記憶體內分析。 |
| **儲存體最佳化** | 高磁片輸送量和 i/o。 | 適用於巨量資料、SQL 與 NoSQL 資料庫。 |
| **GPU 最佳化** | 特製化 VM。 一或多個 GPU。 | 繁重圖形與視訊編輯。 |
| **高效能** | 最快、最強的 CPU。 具有選用高輸送量網路介面 (RDMA) 的 VM | 關鍵高效能應用程式。 |

- 請務必了解這些 VM 之間的定價差異，以及長期預算影響。
- 每個類型中都有一些 VM 系列。
- 此外，當您選取某個系列中的 VM 時，您只能在該系列中相應增加或相應減少 VM。 例如， `DS2_v2` 實例可以相應增加至 `DS4_v2` ，但無法變更為不同數列（例如實例）的實例 `F2s_v2` 。

**瞭解更多資訊：**

- 深入瞭解[VM 類型和大小調整](https://docs.microsoft.com/azure/virtual-machines/windows/sizes)，並將大小對應至類型。
...
| **磁碟** | 以分頁 Blob 為基礎。 <br><br> 磁片類型：標準 (HDD 或 SSD) 或 premium (SSD) 。 <br><br> 磁片管理：非受控 (您管理磁片設定和存放裝置) 或受控 (您選取磁片類型，而 Azure 會為您管理磁片) 。 | 使用適用于 Vm 的 premium 磁片。 使用受控磁碟來獲得簡單的管理與規模調整。 |
| **佇列** | 透過 HTTP 或 HTTPS)  (，儲存和抓取透過驗證的呼叫存取的大量訊息。 | 將應用程式元件與非同步訊息佇列連接。 |
...
| 帳戶類型 | 詳細資料 | 使用方式 |
| --- | --- | --- |
| **一般用途 v2 標準** | 支援 Blob (區塊、分頁、附加)、檔案、磁碟、佇列與資料表。 <br><br> 支援經常性、非經常性和封存存取層。 支援 ZRS。 | 用於大部分案例與大部分類型的資料。 標準儲存體帳戶可以是 HDD 或 SSD 型。 |
...
| 類型 | 詳細資料 | 使用方式 |
| --- | --- | --- |
| **本地備援儲存體 (LRS)** | 透過在單一儲存體單位中複寫到不同的容錯網域與更新網域，來在本地服務故障時維持可用性。 在一個資料中心存放您資料的多個複本。 在指定的一年中，至少提供 99.999999999% (第11個物件的) 持久性。 | 請考慮您的應用程式是否儲存可輕鬆重建的資料。 |
| **區域備援儲存體 (ZRS)** | 透過複寫到單一區域中的三個儲存體叢集，來維持在資料中心無法使用時的可用性。 每個存放裝置叢集會實體分隔並位於自己的可用性區域中。 藉由在多個資料中心或區域中保留多個資料複本，在指定的一年內提供至少 99.9999999999% (12 9 的) 持久性。 | 當您需要一致性、持久性與高可用性時可以考 慮使用。 當多個區域都受到永久影響時，可能無法針對區域性災害提供保護。 |
| **異地備援儲存體 (GRS)** | 透過將資料複寫到與主區域距離數百公里遠的次區域，來在提供整個區域的服務都無法使用時的可用性。 在指定的一年內，至少提供 99.99999999999999% (十六9的物件) 持久性。 | 除非 Microsoft 起始容錯移轉到次區域的作業，否則複本資料無法使用。 若發生容錯移轉，讀取與寫入存取可用。 |
...
- 瞭解[azure 匯入/匯出，以將大量資料移轉至 azure blob 和](https://docs.microsoft.com/azure/storage/common/storage-import-export-service)檔案。
...
![Azure 虛擬機器保留實例 ](./media/migrate-best-practices-costs/reserve.png)
 _Azure 保留的 VM 執行個體。_
...
若要這樣做，您可以使用 Azure 成本管理和計費 API。 接著，將資料彙總到單一來源 (例如 Azure SQL) 之後，您可以使用諸如 Power BI 的工具來呈現彙總的資料。 您可以建立彙總的訂用帳戶報告，以及精細的報告。 例如，對於需要主動深入解析成本管理的使用者，您可以根據部門、資源群組或其他資訊來建立特定的成本觀點。 您不需要將完整的 Azure 帳單資料存取權提供給他們。
...
本節中的最佳作法包括使用 Azure 成本管理和計費來進行成本預算和分析、監視資源及執行資源群組預算，以及優化監視、儲存體和 Vm。

## <a name="best-practice-use-azure-cost-management-and-billing"></a>最佳做法：使用 Azure 成本管理和計費

Microsoft 提供 Azure 成本管理和計費，協助您追蹤費用：
...
如果您遵循在遷移前選取儲存體的最佳做法，您可能會享受某些優點。 但是，您仍然可以優化額外的儲存成本。 Blob 與檔案會隨著時間而變成過時。 資料可能無法再使用，但法規需求可能表示您必須將它保存一段時間。 因此，您可能不需要將它存放在用於原始移轉的高效能儲存體上。
...
- 預算 API 可以與 Azure Logic Apps 和 runbook 整合。
