---
title: 將應用程式遷移至 Azure App Service 和 SQL Database
description: 使用適用于 Azure 的雲端採用架構，瞭解如何藉由將應用程式遷移至 Azure App Service 和 Azure SQL Database 來重構應用程式。
...
ms.locfileid: "101791899"
---
<!-- cSpell:ignore WEBVM SQLVM contosohost vcenter contosodc smarthotel SHWEB SHWCF -->

# <a name="migrate-an-application-to-azure-app-service-and-sql-database"></a>將應用程式遷移至 Azure App Service 和 SQL Database

本文示範虛構公司 Contoso 如何在遷移至 Azure 的過程中，重構在 VMware Vm 上執行的兩層式 Windows .NET 應用程式。 Contoso 團隊將 (VM) 的應用程式前端虛擬機器遷移至 Azure App Service web 應用程式，並將應用程式資料庫移轉至 Azure SQL Database。
...
- 針對應用程式的資料庫層，Contoso 會藉由參考 [功能比較： AZURE Sql database 和 AZURE Sql 受控實例](/azure/azure-sql/database/features-comparison)，將 Azure SQL DATABASE 與 sql Server 進行比較。 Contoso 決定使用 Azure SQL Database 的原因有幾個：
  - Azure SQL Database 是受控關聯式資料庫服務。 其在多個服務層級上提供可預測的效能，而且幾乎免管理。 優點包括無須停機的動態延展性、內建智慧最佳化及全球延展性和可用性。
  - Contoso 可以使用輕量資料移轉小幫手來評估內部部署資料庫移轉至 Azure SQL Database。
  - Contoso 可以使用 Azure 資料庫移轉服務，將內部部署資料庫移轉至 Azure SQL Database。
  - 透過軟體保證，Contoso 可以使用適用于 SQL Server 的 Azure 混合式權益，在 SQL Database 中的資料庫上以折扣優惠交換現有授權。 這種方法可以提供最高達30% 的成本節約。
  - SQL Database 提供安全性功能，例如永遠加密、動態資料遮罩、資料列層級安全性，以及 SQL 威脅偵測。
...
| [資料移轉小幫手](/sql/dma/dma-overview) | Contoso 會使用資料移轉小幫手來評定和偵測可能影響 Azure 中資料庫功能的相容性問題。 資料移轉小幫手會評估 SQL 來源與目標之間的功能同位，並建議效能和可靠性的改進。 | 這是可下載的免費工具。 |
...
| [Azure SQL Database](/azure/azure-sql/database/sql-database-paas-overview) | 完全受控的智慧型關聯式雲端資料庫服務。 | 成本是以功能、輸送量和大小為基礎。 [深入了解](https://azure.microsoft.com/pricing/details/azure-sql/sql-managed-instance/single/)。 |
...
> - **步驟3：評估資料庫**。 Contoso 會使用資料移轉小幫手來評估要遷移的資料庫，然後透過 Azure 資料庫移轉服務進行遷移。
...
> - **步驟6：在 Azure DevOps 中設定組建和發行管線**。 在最後一個步驟中，Contoso 會在 Azure DevOps 中設定組建和發行管線來建立應用程式，然後將其部署至兩個不同的 web 應用程式。

## <a name="step-1-assess-and-migrate-the-web-apps"></a>步驟1：評定及遷移 web 應用程式

Contoso 管理員會使用 [Azure App Service](https://azure.microsoft.com/migration/web-applications/) 的「遷移小幫手」工具來評估及遷移其 web 應用程式。 他們使用將 [ASP.NET 應用程式遷移至 Azure 學習路徑](/learn/paths/migrate-dotnet-apps-azure/) ，作為程式期間的指南。 系統管理員會執行下列動作：
...
## <a name="step-2-provision-a-database-in-azure-sql-database"></a>步驟2：在 Azure SQL Database 中布建資料庫
...
1. 他們會指定資料庫名稱，以符合 `SmartHotel.Registration` 在內部部署 VM 上執行的資料庫。 他們會將資料庫放在 `ContosoRG` 資源群組中。 這是用於 Azure 中生產資源的資源群組。

    ![顯示 SQL Database 實例詳細資料的螢幕擷取畫面。](./media/contoso-migration-refactor-web-app-sql/provision-sql2.png)

1. 他們會 `sql-smarthotel-eus2` 在主要區域中設定新的 SQL Server 實例。
...
1. 他們會開啟資料庫，並記下它們在使用資料移轉小幫手進行遷移時需要的詳細資料。

    ![資料庫實例文字檔的螢幕擷取畫面。](./media/contoso-migration-refactor-web-app-sql/provision-sql6.png)

**需要其他協助？**

- [取得](/azure/azure-sql/database/single-database-create-quickstart) 提供 SQL database 的協助。
- 瞭解 [vCore 資源限制](/azure/azure-sql/database/resource-limits-vcore-elastic-pools)。

## <a name="step-3-assess-the-database"></a>步驟3：評估資料庫

Contoso 管理員會使用資料移轉小幫手來評估資料庫，然後藉由參考 [逐步執行遷移教學](/azure/dms/tutorial-sql-server-to-azure-sql)課程，使用 Azure 資料庫移轉服務來進行遷移。 他們可以執行線上、離線和混合式 (預覽版) 的遷移。

簡單來說，系統管理員會執行下列動作：

- 他們使用資料移轉小幫手來探索和解決任何資料庫移轉問題。
- 他們會使用連線到虛擬網路的 Premium SKU 來建立 Azure 資料庫移轉服務實例。
- 它們可確保實例可以透過虛擬網路存取遠端 SQL Server。 這需要確保在虛擬網路層級、網路 VPN 和裝載 SQL Server 的電腦上，允許從 Azure 到 SQL Server 的所有連入埠。
- 他們會設定實例：
  - 建立移轉專案。
  - 將來源 (內部部署資料庫) 。
  - 選取目標。
...
1. 他們會開啟應用程式的方案檔。 Web 應用程式和 WCF 服務在檔案中有不同的專案。
    ...
    ![解決方案瀏覽器的螢幕擷取畫面，其中列出 web 應用程式和 WCF 服務專案。](./media/contoso-migration-refactor-web-app-sql/vsts4.png)
...
1. 在 Visual Studio 中，系統管理員會 `SmartHotel.Registration.wcf` 從方案檔中開啟專案。 在專案中，他們會 `connectionStrings` 使用連接字串來更新檔案的區段 `web.config` 。
     ...
     ![' SmartHotel. wcf ' 專案中 'web.config' 檔案的 ' connectionStrings ' 區段的螢幕擷取畫面。](./media/contoso-migration-refactor-web-app-sql/string2.png)

1. 他們會將的檔案區段變更為 `client` `web.config` `SmartHotel.Registration.Web` 指向 WCF 服務的新位置。 這是裝載服務端點的 WCF web 應用程式的 URL。

    ![' SmartHotel ' 專案中 'web.config' 檔案用戶端區段的螢幕擷取畫面。](./media/contoso-migration-refactor-web-app-sql/strings3.png)
...
1. 他們會選取 [ **Azure 存放庫 Git** ]，然後在 [存放庫] 下拉式清單中選取相關的存放 **庫** 。

    ![[Azure 存放庫 Git] 按鈕和所選存放庫的螢幕擷取畫面。](./media/contoso-migration-refactor-web-app-sql/pipeline2.png)

1. 在 [ **選取範本**] 下，他們會選取 `ASP.NET` 組建的範本。
...
1. 他們會使用組建的名稱 `ContosoSmartHotelRefactor-ASP.NET-CI` ，然後選取 [ **儲存] & 佇列**，以啟動第一個組建。
     ...
     ![組建的 [儲存 & 佇列] 按鈕的螢幕擷取畫面。](./media/contoso-migration-refactor-web-app-sql/pipeline4.png)
...
1. 在管線 > **構件** 上，他們會選取 [ **+ 新增** 成品]，然後選取要使用 `ContosoSmarthotel360Refactor` 管線建立。
...
1. 在 [ **選取檔案或資料夾**] 中，展開 [ **放置** ] 資料夾，選取組建期間建立的檔案，然後 `SmartHotel.Registration.Wcf.zip` 選取 [ **儲存**]。
    ...
    ![選取 [選取檔案或資料夾] 窗格以選取 WCF 檔案的螢幕擷取畫面。](./media/contoso-migration-refactor-web-app-sql/pipeline16.png)

1. 他們會選取 [**管線**  >  **階段**]，然後選取 [ **+ 新增**] 以新增的環境 `SHWEB-EUS2` 。 他們會選取另一個 Azure App Service 部署。

    ![用於新增環境的 * * 1 工作、1工作 * * 連結的螢幕擷取畫面。](./media/contoso-migration-refactor-web-app-sql/pipeline17.png)
...
- Contoso 小組會檢查 Azure SQL Database 的備份需求。 [深入了解](/azure/azure-sql/database/automated-backups-overview)。
...
在本文中，Contoso 會將應用程式前端 VM 遷移至兩個 Azure App Service web apps，以重構 Azure 中的 SmartHotel360 應用程式。 應用程式資料庫已遷移至 Azure SQL Database。
