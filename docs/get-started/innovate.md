...
在雲端中建立新的產品和服務時，需要不同于遷移的方法。 Azure 雲端採用架構中的創新方法會建立一種方法，以引導新產品和服務的開發。
...
為避免常見的封鎖程式，請為創新建立清楚且簡潔的商務策略。 專案關係人在動機和預期的商務成果方面的調整，是由雲端採用小組所做出的決策。
...
在第一次建立商業案例時，評估潛在雲端採用工作的初始高階報酬率。 此步驟的目標是要確保所有專案關係人都符合一個簡單的問題：根據可用的資料，雲端的整體採用是明智的商業決策嗎？ 根據該問題，小組可以更妥善地配合此創新專案如何協助滿足使用者在採用雲端時的預估需求。
...
在大部分的企業中，可以透過使用現有資產（例如應用程式、Vm 和資料）來加速創新。 當您規劃創新時，請務必瞭解這些資產遷移至雲端的方式和時間。

**項**

- 現有清查上的原始資料，例如應用程式、Vm 和資料。
...
探索和評量提供更深入的技術調整層級，可協助您建立動作計畫，讓您用來遷移計畫創新所需的任何相依工作負載。 當公司擁有現有的資料來源、集中式應用程式或必要的服務層級，以在企業其他部分的內容中提供創新時，此案例就很常見。 當有相依的系統時，下列文章可以引導您進行探索和評估。
...
- 完成[Microsoft Azure 架構完善的審查](https://docs.microsoft.com/assessments/?id=azure-architecture-review)，以評估各種架構和作業決策。
...
裝載于雲端的所有資產都在登陸區域內。 該登陸區域可能會有明確的治理、安全性和操作需求。 或者，這可能是全新的訂用帳戶，不支援其他小組。 在任一案例中，請務必從一開始就遵循治理和操作需求的登陸區域開始。 從核准的登陸區域開始，可協助您的小組及早探索原則違規的開發，而不是將解決方案發行到生產環境。 早期探索可協助移除封鎖器，並為採用小組和治理小組提供實施變更所需的時間。
...
雲端採用架構是生命週期解決方案。 它可協助您開始創新的旅程。 它也可協助您提升支援創新工作的團隊成熟度。 下列小組可以使用這些後續步驟來繼續提升其工作的成熟度。 這些平行處理常式不是線性的，而且不應該被視為封鎖器。 相反地，每個都是平行值串流，以協助您的公司整體雲端就緒程度成熟。
