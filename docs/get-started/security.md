...
本指南中的步驟適用于所有支援雲端環境和登陸區域安全性保證的角色。 這包括立即風險緩和優先順序、建立現代化安全性策略的指引、運用方法，以及在該策略上執行。

本快速入門手冊中包含跨雲端採用架構的各種元素：

![企業安全性入門](../_images/get-started/security-map.png)

遵循本指南中的步驟可協助您整合程式中重要點的安全性，以避免雲端採用的障礙，並減少不必要的業務或操作中斷。

Microsoft 已建立功能和資源，可協助您在 Microsoft Azure （本檔中參考）上加速執行此安全性指引。 這些資源是設計來協助您建立、監視和強制執行安全性，而且經常更新和審核。

此圖顯示 Microsoft 建議使用安全性指引和平臺工具，在 Azure 中建立雲端資產的安全性可見度和控制的整體方法。
...
除了符合任何明確的法規合規性需求之外，我們建議下列步驟，以解決大部分組織移到雲端時所面臨的最大安全性挑戰。
...
- **進程：** 藉由指派安全性角色和責任，並建立事件回應程式，來啟用快速的安全性決策和持續改進。
...
- **人員：** 為安全性小組提供在轉換至雲端環境時成功部署和操作所需的教育、工具和存取權。
  ...
  - 針對他們所使用的平臺，**訓練技術人員**關於雲端安全性功能的技術詳細資料。 Microsoft 提供廣泛的[Azure 安全性檔案](https://docs.microsoft.com/azure/security)。
- **長期架構決策：** 以適當的決策建立長期基礎。 這在稍後變更時非常艱難且昂貴。
...
在此初始步驟中，治理小組也應開始協調可跨環境監視、管理及強制執行的安全性基準建立。 以下的步驟4將討論如何建立此功能的其他指引。

> [!NOTE]
> 每個組織都應該定義其專屬的最低標準，因為風險狀態和後續對該風險的承受度可能會根據產業、文化特性和其他因素而有很大的差異。 例如，銀行可能無法容忍其對其信譽的任何潛在損害，即使是測試系統的輕微攻擊，有些組織會在加速其數位轉型為三到六個月時接受相同的風險。

## <a name="step-2-modernize-security-strategy"></a>步驟2：將安全性策略現代化

雲端中的有效安全性需要音效策略，以反映目前的威脅環境，以及裝載企業資產的雲端平臺本質。 明確的策略可改善所有小組的工作，以提供安全且可持續的企業雲端環境，以降低業務風險。 安全性策略必須啟用已定義的商務結果、降低風險，使其成為可接受的層級，並讓員工具有生產力。

雲端安全性策略為所有工作團隊提供清楚的指引，適用于在此採用的技術、流程和人員。 策略應該會通知雲端架構和技術功能、引導安全性架構和功能，以及影響小組的訓練和教育。

**項**

策略步驟應該會產生一份檔，輕鬆地傳達給組織內的許多專案關係人，可能包括組織領導小組的主管。

因此，我們建議您在簡報中捕捉策略，以方便進行討論和更新。 視文化特性和喜好設定而定，檔也可以支援這種方式。

- **策略簡報：** 您可能會有單一策略簡報，或者您也可以選擇為領導物件建立摘要版本。
  - **完整簡報：** 這應該包含主要簡報或選擇性參考投影片中安全性原則的一組完整元素。
  - **執行摘要：** 與資深主管和麵板成員搭配使用的版本，其中僅包含與其角色相關的重要元素，例如風險胃口、最高優先順序或已接受的風險。
- 您也可以在[策略和計畫範本](https://archcenter.blob.core.windows.net/cdn/fusion/readiness/Microsoft-Cloud-Adoption-Framework-Strategy-and-Plan-Template.docx)中記錄動機、結果和商業理由。

**建立安全性策略的最佳作法：**

成功的程式會將這些元素併入其安全性策略流程中：

- **密切配合商務策略：** 安全性的職責是保護商業價值，因此將所有安全性工作與該目的保持一致，並將內部衝突降到最低，是不可或缺的。
  - **建立共用的理解：** 企業、IT 和安全性需求。
  - **及早將安全性整合到雲端採用：** 若要避免最後一分鐘危機肇因風險。
  - **Agile 方法：** 立即建立最低安全性需求，並持續改善一段時間的安全性保證。
  - **安全性文化特性變更：** 透過刻意積極的領導行動。
...
**策略核准：** 負責業務成果或組織內商務線業務風險的主管與業務領導人（這可能包括根據組織的主管）。

## <a name="step-3-develop-a-security-plan"></a>步驟3：開發安全性計畫

規劃會藉由定義結果、里程碑、時程表和工作擁有者，將安全性策略放在動作中。 此方案也會概述小組的角色和責任。

安全性規劃和雲端採用規劃不應在隔離的情況中完成。 請務必及早邀請雲端安全性小組進入規劃週期，以避免發生工作停止或提高安全性問題的風險。 安全性規劃最適合用來深入瞭解並感知數位資產和現有的 IT 組合，這是完全整合到雲端規劃程式。

**項**

- **安全性計畫：** 應該是雲端主要規劃檔的一部分，而且可能是使用[策略和計畫範本](https://archcenter.blob.core.windows.net/cdn/fusion/readiness/Microsoft-Cloud-Adoption-Framework-Strategy-and-Plan-Template.docx)的檔、詳細的投影片、專案檔，或這些格式的組合，視組織的大小、文化特性和標準實務而定。

- 安全性計畫應包含所有這些元素：

  - **組織函數計畫：** 因此，小組知道目前的安全性角色和責任如何隨著移至雲端而改變。
  ...
  Microsoft 提供參考架構和技術功能，可在您建立架構和藍圖時協助您，包括：
    ...
    - [Microsoft 網路安全性參考架構](https://aka.ms/mcra)，針對橫跨內部部署和雲端資源的混合式企業，建立整體的網路安全性架構。
    ...
    - [Azure 資訊安全中心](https://docs.microsoft.com/azure/security-center/)和[Microsoft cloud app Security](https://docs.microsoft.com/cloud-app-security/)來保護雲端資產。
  - **安全性認知和教育計畫：** Aso 所有小組都有基本的重要安全性知識。
  - **資產敏感度標記：** 使用符合業務影響的分類法來指定敏感性資產（由商務專案關係人、安全性小組和其他感興趣的合作物件共同建立）。

- **假設混合式環境：** 其中包括軟體即服務（SaaS）應用程式、內部部署環境，以及多個雲端基礎結構即服務和平臺即服務提供者（如果適用）。
- **使用安全性變更來更新雲端方案：** 更新主要雲端採用方案的其他區段，以反映安全性計畫所觸發的變更。

**安全性規劃的最佳作法：** 如果您的規劃採用下列方法，您的安全性計畫可能會更成功：

- **Agile 安全性採用：** 先建立最低安全性需求，並將所有非關鍵專案移至後續步驟的優先順序清單。
這不應該是傳統的詳細3-5 年度計畫，因為雲端和威脅環境的變更速度太快，使這類計畫變得很有用。 您的計畫應著重于開發開始步驟和結束狀態：
  - **快速獲勝：** 適用于立即取得快速勝利和開始長期計畫的詳細計畫，將會產生高影響力。 這可能是3-12 個月，視組織文化、標準實務和其他因素而定。
  - **清除**所需結束狀態的願景，以引導每個小組的規劃程式（這可能需要數年才能達成）。
- **廣泛共用方案：** 提升專案關係人對的認知、意見反應和購買。
- **符合策略性成果：** 請確定您的計畫符合並完成安全性策略中所述的策略性結果
- **擁有權、責任和期限：** 請確定已識別每個工作的擁有者，並已認可在特定時間範圍內完成該工作。
- **連接安全性的人：** 若要在這段期間轉換和新的期望，請：
  - **主動支援小組成員轉換：** 有清楚的溝通與指導：
    - 他們需要學習哪些技能
    - 為何需要瞭解它們（以及這麼做的優點）
    - 如何取得這種知識（並提供協助他們瞭解的資源）。
  這可以使用[策略和計畫範本](https://archcenter.blob.core.windows.net/cdn/fusion/readiness/Microsoft-Cloud-Adoption-Framework-Strategy-and-Plan-Template.docx)來記載，而且您可以使用[線上 Microsoft 安全性訓練](https://docs.microsoft.com/security/compass/microsoft-security-compass-introduction)來協助您的小組成員教育。
  - **讓安全性意識吸引人：** 協助人員真的與組織保持安全的部分聯繫。
- **查看 Microsoft 學習和指引：** Microsoft 已發佈見解和觀點，協助您的組織規劃其對雲端的轉換，以及一套現代化的安全性策略，包括記錄的訓練、檔和安全性最佳做法，以及建議的標準。
...
**安全性計畫核准：** 安全性領導小組（CISO 或同等資格）。

## <a name="step-4-secure-new-workloads"></a>步驟4：保護新的工作負載

以安全的狀態啟動，比稍後在您的環境中)) 安全性來得容易許多。 我們強烈建議您從安全的設定開始，以確保工作負載會在安全的環境中遷移及開發和測試。
...
- 確保新的登陸區域符合組織的合規性與安全性需求。

**支援交付後完成的指引：**

- **Blend 現有需求和雲端建議：** 從建議的指引開始，然後根據您的獨特安全性需求進行調整。 我們發現嘗試強制執行現有內部部署原則和標準的挑戰，因為這通常是指過時的技術或安全性方法。 Microsoft 已發佈指引以協助您建立安全性基準：
...
- **提供護欄：** 這些應該要有自動化的原則審核和強制執行。 針對這些新環境，小組應該致力於審查和強制執行貴組織的安全性基準，以將開發期間的安全性意外降到最低，以及持續整合和持續部署（CI/CD）工作負載。
Microsoft 在 Azure 中提供了數個原生功能來啟用：
  - [安全分數](https://docs.microsoft.com/azure/security-center/secure-score-security-controls)：對您的 Azure 安全性狀態進行評分評估，可讓您追蹤組織中的安全性工作和專案。
  ...
  - [Azure 原則](https://docs.microsoft.com/azure/governance/policy/)：這些其他服務所使用之可見度和控制項功能的基礎。 Azure 原則已整合到[Azure Resource Manager （ARM）](https://docs.microsoft.com/azure/azure-resource-manager)中，可讓您在 Azure 中的任何資源之前、期間或建立之後，對其進行各種修改和強制執行原則。
- [改善登陸區域作業](../ready/considerations/landing-zone-security.md)：使用最佳作法來改善指定登陸區域內的安全性。
...
許多組織已將資產部署至企業雲端環境，但不會套用安全性最佳作法，因而提高業務風險。
...
- 使用安全性基準原則來測試生產部署的作業是否就緒。
- 驗證遵守安全性基準設計指引和安全性需求。

**支援交付後完成的指引：**

- 使用您在[步驟 4](#step-4-secure-new-workloads)中建立的相同安全性基準作為理想的狀態。 您可能必須將某些原則設定調整為只進行審核，而不是強制執行它們。
- 平衡營運和安全性風險。 由於這些環境可能裝載啟用重要商務程式的生產系統，因此您可能需要以累加方式執行安全性改進，以避免風險的作業停機時間。
- 根據業務重要性來設定探索和補救安全性風險的優先順序，從有高商業影響的工作負載（如果遭到入侵，而且工作負載的風險嚴重暴露于風險）。
...
就像所有現代化的專業領域一樣，安全性是一種反復的程式，應該著重在持續改進的過程中。 如果組織在一段時間內無法承受焦點，則安全性狀態也可能會衰減。

一致的安全性需求應用來自于音效治理專業領域和自動化解決方案。 一旦「雲端安全性小組」定義了「安全性基準」，就應該將這些需求進行審核，以確保它們會一致地套用至所有雲端環境（並在適用的情況下強制執行）。

**項**

- 確保將組織的安全性基準套用到所有相關的系統，並使用[安全分數](https://docs.microsoft.com/azure/security-center/secure-score-security-controls)或類似的機制來審核異常。
- 記錄安全性[基準專業領域範本](../govern/security-baseline/template.md)中的安全性基準原則、程式和設計指引。

**支援交付後完成的指引：**

- 使用您在[步驟 4](#step-4-secure-new-workloads)中所建立的相同安全性基準和審核機制，做為監視基準的技術元件，並由人員和流程式控制制進行補充以確保一致性。
- 確保所有的工作負載和資源都遵循適當的[命名和標記慣例](../ready/azure-best-practices/naming-and-tagging.md)，並使用 Azure 原則特別強調「資料敏感性」標記來[強制執行標記慣例](https://docs.microsoft.com/azure/governance/policy/tutorials/govern-tags)。
...
這些步驟會引導您執行適當的策略、控制項、進程、技能和文化特性，以一致地管理整個企業的安全性風險。
當您繼續進入雲端安全性的作業模式時，請考慮在本快速入門手冊的組建中進行這些後續步驟。

- 請參閱[Microsoft 安全性檔案](https://docs.microsoft.com/security)，以協助安全性專業人員建立及改善網路安全性策略、架構和排定優先順序的藍圖。
- 審查[Azure 服務內建安全性控制](https://docs.microsoft.com/azure/security/fundamentals/security-controls)的安全性資訊。
