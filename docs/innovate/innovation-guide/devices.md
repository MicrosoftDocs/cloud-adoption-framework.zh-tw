---
title: 無所不在的計算和裝置管理
description: 瞭解通用運算和 Azure 的架構，以透過連線的裝置和感知 edge 裝置來建立沉浸式且有效的商務解決方案。
...
keywords: 連接的裝置、無所不在的計算、裝置管理、邊緣裝置
ms.openlocfilehash: 2d098a907ed0b1ce0865a0e52b51b566999d1d15
ms.sourcegitcommit: b8f8b7631aabaab28e9705934bf67dad15e3a179
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 03/03/2021
ms.locfileid: "101787598"
---
<!-- cSpell:ignore umarmohamedusman umarm Moovit -->

# <a name="interact-through-connected-devices"></a>透過連線的裝置進行互動

透過間歇連線的裝置和感知 edge 裝置進行創新。 協調數百萬個這類裝置、取得及處理無限資料，並利用日益增加的多重感應、多重裝置體驗。 對於網路邊緣的裝置，Azure 提供了一個架構供您建置沉浸式且有效的商務解決方案。 您可以透過 Azure 和 AI 技術相結合而實現的通用運算，建置所能想到的各種智慧型應用程式和系統。

無所不在的運算是指將裝置和處理器連接到具有持續可用性的資訊處理，以便在任何需要的地方使用任何連線的裝置或感知 edge 裝置，進行計算和處理。 常見運算的範例包括將資訊傳送至另一個系統的任何系統，以順暢地完成工作，例如會警示是否有來自行動電話的來電，以及允許透過 watch 完成通話，或是學習和調整的系統（例如控溫器或智慧型喇叭）。

Azure 客戶採用一組持續擴展的連線系統和裝置，來收集和分析資料 (接近其使用者、資料，或兩個) ，並具備完整的裝置管理。 使用者可以透過高回應性的內容感知應用程式取得即時深入解析和體驗。 藉由將部分工作負載移至邊緣，這些連線的裝置可以花較少的時間將訊息傳送至雲端，並更快速地回應空間事件。
...
- **內建裝置管理** 和布建，可大規模連接及管理 IoT 和邊緣裝置。
