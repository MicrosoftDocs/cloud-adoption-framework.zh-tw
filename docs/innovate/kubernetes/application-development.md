...
> | **檢查常見的 Kubernetes 案例。** Kubernetes 通常被視為提供微服務的平臺，但它變得更廣泛。 觀看這段影片以瞭解常見的 Kubernetes 案例，例如 batch 分析和工作流程。    | [使用&nbsp;&nbsp;Kubernetes&nbsp;&nbsp;的&nbsp;常見案例（影片）](https://www.youtube.com/watch?v=zd8vYhrFXp4&list=PLLasX02E8BPCrIhFrc_ZiINhbRkYMKdPT&index=7) |
...
> | **設定應用程式調整需求。** Kubernetes 支援水平 Pod 自動調整，可根據 CPU 使用率或其他選取的計量來調整部署中的 Pod 數目。 若要使用自動調整程式，pod 中的所有容器都必須定義 CPU 要求和限制。    | [設定水準 Pod 自動調整程式](https://docs.microsoft.com/azure/aks/tutorial-kubernetes-scale#autoscale-pods) |
