...
雲端採用架構是 Microsoft 在 Azure 中進行雲端採用的方法，此架構會整合並共用來自 Microsoft 員工、合作夥伴和客戶的最佳做法。 此架構為企業客戶提供一組工具、指引和敘述，協助擬定技術、業務和人員策略，以在採用期間推動所需的業務成果。 此指引符合一般雲端採用生命週期的各種階段和反覆項目，以確保在適當的時間可輕易取得正確的指引。

![雲端採用架構概觀](./_images/caf-overview.png)

## <a name="getting-started-executive-summaries"></a>開始使用：執行摘要

對於初次使用雲端採用架構的客戶，我們提供三篇簡介文章：[移轉](./getting-started/migrate.md)、[創新](./getting-started/innovate.md)和[啟用](./getting-started/enable.md)。 每篇文章都會提供執行摘要，以及經歷企業採用生命週期的高階旅程。

如需更具體的指引，請繼續閱讀採用生命週期各個階段的連結。

## <a name="use-the-cloud-adoption-framework-throughout-the-adoption-lifecycle"></a>在整個採用生命週期內使用雲端採用架構

雲端採用架構的每一節都會對應於前述的概觀圖表。 此清單可協助您找出最符合您目前在雲端採用生命週期中所處階段的小節。
...
雲端採用架構目前提供公開預覽版。 當下節所述的期望意圖完全實現時，此架構便達到正式運作 (GA) 階段。 在此預覽版本中，主要焦點區域是：

- [雲端採用策略](./strategy/index.md)
- [雲端採用規劃](./plan/index.md)
...
如果您的企業不熟悉 Azure，請從[開始使用雲端採用架構](./getting-started/migrate.md)開始閱讀。 本概觀包含您企業數位轉型的規範指導方針，逐步說明程序中的每個步驟。

如果您的企業曾體驗過 Azure，且正在尋求更詳細的指引或特定領域的最佳做法，請檢閱[治理概觀](./govern/index.md)和[架構決策指南](./decision-guides/index.md)。 我們將新增更多章節供正式運作方法之用。
