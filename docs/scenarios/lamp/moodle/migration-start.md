...
解壓縮之後，您可以在底下找到 `storage` 目錄 `home/azureadmin` 。 此 `storage` 目錄包含 `moodle` 、 `moodledata` 和 `configuration` 目錄，以及資料庫備份檔案。 您可以在下列步驟中，將每個檔案和目錄複寫到目標位置：

1. 將 `moodle` 和 `moodledata` 目錄複寫到下的共用位置 `/moodle` 。
...
### <a name="update-the-nginx-configuration-variables"></a>更新 NGINX 設定變數
...
如果內部部署 Moodle 應用程式有任何不在控制器 VM 上的 PHP 延伸模組，您可以手動進行安裝。
