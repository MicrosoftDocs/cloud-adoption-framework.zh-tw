...
1. 在 [ **總覽** ] 頁面上，選取 **DNS 名稱** 旁邊的複製圖示。 若要更新檔案 `config.php` ：
...
1. 更新 HTML `root` 目錄位置。 例如，將 `root /var/www/html/moodle;` 更新為 `root /moodle/html/moodle;`。 內部部署根目錄可以位於任何位置。
