...
萬一發生嚴重損壞的情況，RTO 就是將任何系統復原到其嚴重損壞狀態所應採取的時間量。 針對每個工作負載，這會包含還原 Vm 和應用程式的最低必要功能所需的時間。 它也包含還原應用程式所需之資料所需的時間量。
...
如果系統具有以小時或天測量的復原點目標，則系統失敗會導致在最後一個復原點與中斷之間的時間或天數遺失資料。 理論上，1天的 RPO 會導致發生失敗的一天內的所有交易遺失。
...
IT 環境中的大部分工作負載都支援非常小型的商務或技術程式。 對於商務作業沒有系統影響的系統，通常不會保證快速復原所需的投資，或將資料遺失降至最低。 藉由建立基準，企業可以清楚地瞭解在一致且可管理的價位上提供的復原支援層級。 這項瞭解可協助商務專案關係人評估增加的復原投資價值。
...
在雲端管理小組執行平臺作業之前，他們通常會支援平臺即服務（PaaS）資料平臺的改良作業。 比方說，雲端管理小組可以輕鬆地針對 Azure SQL Database 或 Azure Cosmos DB 解決方案強制執行較高的備份或多重區域複寫頻率。 這麼做可讓開發小組輕鬆地現代化其資料平臺，藉此提升 RPO。
...
這些虛擬機器的每一分鐘停機時間都可能等於損失的收益或降低獲利。 當 VM 停機時間直接影響企業的會計績效時，RTO 非常重要。 虛擬機器可以使用複寫至次要網站，並自動復原（稱為熱復原模式的模型）來更快速地復原。 在復原的最高狀態中，可以將虛擬機器複寫到功能完整的次要網站。 這種較昂貴的方法稱為高可用性或熱熱復原模式。
...
符合此管理基準元件之後，小組可以進一步查看，以避免發生[平臺作業](./platform.md)和[工作負載作業](./workload.md)的中斷。
