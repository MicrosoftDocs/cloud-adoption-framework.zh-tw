...
從一開始，Microsoft 的許多小組都已參與 Azure 平臺的開發。 很難以確保此大小和複雜度的專案品質與一致性。 您需要一套健全的程式，定期列舉及執行基本的無功能性需求。

Microsoft 遵循的程式會形成本文所述程式的基礎。

## <a name="understand-the-problem"></a>了解問題

如開始使用[：加速遷移](../get-started/migrate.md)中所述，企業數位轉型的第一個步驟是藉由採用 Azure 來識別要解決的商務問題。 下一個步驟是判斷問題的高階解決方案，例如將工作負載遷移至雲端，或調整現有的內部部署服務以包含雲端功能。 最後，您可以設計和執行解決方案。
...
從高階觀點來看，該程序有兩個階段。 在_必要條件階段_中，會建立需求並對應至支援的服務。 此階段不常發生：可能是每年或引進新作業的時間。 必要條件階段的輸出會在_流程階段_中使用。 流程階段的發生頻率較高，例如每月。
...
1. **分析服務相依性**。 大部分的商務作業都需要多個支援服務之間的協調流程。 請務必瞭解服務之間的相依性，以及透過這些服務的任務關鍵性交易流程。
...
這些活動的輸出是一組服務作業的_計分卡計量_。 計分卡測量準則，例如可用性、擴充性和嚴重損壞修復。 計分卡計量表示您預期服務符合的操作條件。 這些計量可以用適合服務作業的任何資料細微性層級來表示。

計分卡應該以在公司擁有者與工程人員之間促使有意義之決策的簡單方式表示。 例如，用於擴充性的計分卡度量可能會以簡單的方式進行色彩編碼。 綠色表示符合已定義的準則，黃色表示無法符合已定義的準則，但會主動實行計畫的補救，而紅色則表示無法在沒有計劃或動作的情況下符合定義的準則。
...
3. **實作補救措施**。 在企業擁有者和工程小組同意計畫之後，請將它實行。 當您查看計分卡計量時，報告執行的狀態。
...
- **商務擁有者：** 提供企業知識，以識別每個任務關鍵性商務作業並設定其優先順序。 此角色也會比較業務影響的風險降低成本，並推動有關補救的最後決策。

- **商業提倡者：** 將商務營運細分為部分，並將這些元件對應至服務和基礎結構，不論是在內部部署或雲端。 此角色需要與每個商業作業關聯的深入技術知識。

- **工程擁有者：** 執行與商務作業相關聯的服務。 這些人員可能會參與設計、執行及部署任何解決方案，以因應審查小組所發現的無功能性需求問題。

- **服務擁有者：** 操作企業的應用程式和服務。 這些個人會收集這些應用程式與服務的記錄與使用狀況資料。 這項資料可用於識別問題，並在部署之後驗證修正程式。

## <a name="review-meeting"></a>審查會議

我們建議您的審核小組定期符合。 例如，小組可能符合每月，然後將狀態和計量報告為每季的資深領導地位。
...
4. 小組會建立新商務作業的計分卡計量。 計分卡包含步驟2中所識別之每個服務的組成元件清單。 它與非功能性需求一致，並包含每個元件符合需求的量值。

5. 對於無法符合非功能性需求的構成元件，小組會設計一個高階解決方案，並指派工程擁有者。 此時，企業擁有者和業務提倡者會根據商務營運的預期收益，建立補救工作的預算。
