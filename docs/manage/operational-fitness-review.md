...
從高階觀點來看，該程序有兩個階段。 在*必要條件階段*中，會建立需求並對應至支援的服務。 此階段不常發生：可能是每年或引進新作業的時間。 必要條件階段的輸出會在*流程階段*中使用。 流程階段的發生頻率較高，例如每月。
...
這些活動的輸出是一組服務作業的*計分卡計量*。 計分卡測量準則，例如可用性、擴充性和嚴重損壞修復。 計分卡計量表示您預期服務符合的操作條件。 這些計量可以用適合服務作業的任何資料細微性層級來表示。
...
- **商務擁有**者：提供企業知識，以識別每個任務關鍵性商務作業並設定其優先順序。 此角色也會比較業務影響的風險降低成本，並推動有關補救的最後決策。
...
- **服務擁有**者：操作商務的應用程式和服務。 這些個人會收集這些應用程式與服務的記錄與使用狀況資料。 這項資料可用於識別問題，並在部署之後驗證修正程式。
