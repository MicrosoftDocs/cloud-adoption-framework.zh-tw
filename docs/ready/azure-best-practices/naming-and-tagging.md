...
| API 管理服務實例 | 全球                             | apim- \< 應用程式或服務名稱\>                                | apim-navigator-生產環境                                                                                                     |
...
| DNS 標籤                    | 全球          | \<M 的記錄\>.[\<區域\>.cloudapp.azure.com]                   | <li> dc1.westus.cloudapp.azure.com <li> web1.eastus2.cloudapp.azure.com |
...
| VM 儲存體帳戶          | 全球         | stvm \< 效能類型 \> \< 應用程式名稱或生產名稱 \> \< 區域\>\<\#\#\#\> | <li> stvmstcoreeastus2001 <li> stvmpmcoreeastus2001 <li> stvmstplmeastus2001 <li> stvmsthadoopeastus2001 |
| Web 應用程式                     | 全球         | 應用程式 \< 應用程式名稱 \> - \< 環境 \> - \< \# \# \# \> 。 [{azurewebsites.net}]   | <li> app-navigator-prod-001.azurewebsites.net <li> app-accountlookup-dev-001.azurewebsites.net |
| 函式應用程式                | 全球         | func- \< 應用程式名稱 \> - \< 環境 \> - \< \# \# \# \> 。 [{azurewebsites.net}]  | <li> func-navigator-prod-001.azurewebsites.net <li> func-accountlookup-dev-001.azurewebsites.net |
| 雲端服務               | 全球         | 可能是 \< 應用程式名稱 \> - \< 環境 \> - \< \# \# \# \> 。 [{cloudapp.net}]        | <li> could-navigator-prod-001.azurewebsites.net <li> could-accountlookup-dev-001.azurewebsites.net |
| 通知中樞            | 資源群組 | pubnames.ntf- \< 應用程式名稱 \> - \< 環境\>                                    | <li> pubnames.ntf-navigator-生產環境 <li> pubnames.ntf-排放-開發 |
| 通知中樞命名空間 | 全球         | ntfns- \< 應用程式名稱 \> - \< 環境\>                                  | <li> ntfns-navigator-生產環境 <li> ntfns-排放-開發 |

### <a name="example-names-databases"></a>範例名稱：資料庫

| 資產類型                     | 領域              | 格式                                 | 範例                                                                  |
|--------------------------------|--------------------|----------------------------------------|---------------------------------------------------------------------------|
| Azure SQL Database 伺服器      | 全球             | sql- \< 應用程式名稱 \> - \< 環境\>       | <li> sql-navigator-生產環境 <li> sql-排放-開發 |
| Azure SQL 資料庫             | Azure SQL Database | sqldb- \< 資料庫名稱>- \< 環境\> | <li> sqldb-使用者-生產環境 <li> sqldb-使用者-開發人員 |
| Azure Cosmos DB 資料庫       | 全球             | cosmos- \< 應用程式名稱 \> - \< 環境\>    | <li> cosmos-navigator-生產環境 <li> cosmos-排放-開發 |
| Azure Cache for Redis 實例 | 全球             | redis-\<應用程式名稱\>-\<環境\>     | <li> redis-navigator-prod <li> redis-emissions-dev |
| MySQL 資料庫                 | 全球             | mysql-\<應用程式名稱\>-\<環境\>     | <li> mysql-navigator-prod <li> mysql-emissions-dev |
| PostgreSQL 資料庫            | 全球             | psql- \< 應用程式名稱 \> - \< 環境\>      | <li> psql-navigator-生產環境 <li> psql-排放-開發 |
| Azure SQL 資料倉儲       | 全球             | sqldw-\<應用程式名稱\>-\<環境\>     | <li> sqldw-navigator-prod <li> sqldw-emissions-dev |
...
| 資產類型                        | 領域  | 格式                                                                        | 範例                                                              |
|-----------------------------------|--------|-------------------------------------------------------------------------------|-----------------------------------------------------------------------|
| 儲存體帳戶（一般用途）     | 全球 | st\<儲存體名稱\>\<\#\#\#\>                                                  | <li> stnavigatordata001 <li> stemissionsoutput001 |
| 儲存體帳戶（診斷記錄） | 全球 | stdiag\<訂用帳戶名稱的前兩個字母和數字\>\<區域\>\<\#\#\#\> | <li> stdiagsh001eastus2001 <li> stdiagsh001westus001 |
| Azure StorSimple                  | 全球 | ssimp\<應用程式名稱\>\<環境\>                                              | <li> ssimpnavigatorprod <li> ssimpemissionsdev |

### <a name="example-names-ai-and-machine-learning"></a>範例名稱： AI 和 Machine Learning

| 資產類型                       | 領域          | 格式                            | 範例                                                          |
|----------------------------------|----------------|-----------------------------------|-------------------------------------------------------------------|
| Azue 認知搜尋           | 全球         | srch-\<應用程式名稱\>-\<環境\> | <li> srch-navigator-prod <li> srch-emissions-dev |
...
| 資產類型                  | 領域          | 格式                              | 範例                                                              |
|-----------------------------|----------------|-------------------------------------|-----------------------------------------------------------------------|
| Azure Data Factory          | 全球         | adf- \< 應用程式名稱 \> \< 環境\>     | <li> adf-navigator-生產環境 <li> adf-排放-開發 |
| Azure 串流分析      | 資源群組 | asa-\<應用程式名稱\>-\<環境\>    | <li> asa-navigator-prod <li> asa-emissions-dev |
| Data Lake Analytics 帳戶 | 全球         | dla\<應用程式名稱\>\<環境\>      | <li> dlanavigatorprod <li> dlaemissionsdev |
| Data Lake Storage 帳戶   | 全球         | dls\<應用程式名稱\>\<環境\>      | <li> dlsnavigatorprod <li> dlsemissionsdev |
| 事件中樞                   | 全球         | evh-\<應用程式名稱\>-\<環境\>    | <li> evh-navigator-prod <li> evh-emissions-dev |
| HDInsight-HBase 叢集   | 全球         | hbase- \< 應用程式名稱 \> - \< 環境\>  | <li> hbase-navigator-生產環境 <li> hbase-排放-開發 |
| HDInsight-Hadoop 叢集  | 全球         | hadoop- \< 應用程式名稱 \> - \< 環境\> | <li> hadoop-navigator-生產環境 <li> hadoop-排放-開發 |
| HDInsight-Spark 叢集   | 全球         | spark- \< 應用程式名稱 \> - \< 環境\>  | <li> spark-導覽器-生產環境 <li> spark-排放-開發  |
| IoT 中樞                     | 全球         | iot- \< 應用程式名稱 \> - \< 環境\>    | <li> iot-navigator-生產環境 <li> iot-排放-開發 |
| Power BI Embedded           | 全球         | pbi- \< 應用程式名稱 \> \< 環境\>     | <li> pbi-navigator-生產環境 <li> pbi-排放-開發 |

### <a name="example-names-integration"></a>範例名稱：整合

| 資產類型        | 領域       | 格式                                                     | 範例                                                      |
|-------------------|-------------|------------------------------------------------------------|---------------------------------------------------------------|
| 服務匯流排       | 全球      | sb-\<應用程式名稱\>-\<環境\>.[{servicebus.windows.net}] | <li> sb-navigator-prod <li> sb-emissions-dev |
