---
title: 使用 Azure 訂用帳戶進行調整
description: 使用適用于 Azure 的雲端採用架構，瞭解如何開發使用多個 Azure 訂用帳戶進行調整的策略。
author: alexbuckgit
ms.author: abuck
ms.date: 05/20/2019
ms.topic: guide
...
組織通常需要多個 Azure 訂用帳戶，做為資源限制和其他治理考慮的結果。 定義調整訂閱的策略非常重要。
...
如果您只有幾個訂用帳戶，獨立管理這些訂用帳戶相當簡單。 但如果您有許多訂用帳戶，請建立管理群組階層，以簡化您的訂用帳戶和資源管理。
...
當您定義管理群組階層時，您會先建立根管理群組。 然後將目錄中所有現有的訂用帳戶移到根管理群組。 新的訂用帳戶一律會建立在根管理群組中。 您之後可以將這些訂用帳戶移至另一個管理群組。

當您將訂用帳戶移至現有的管理群組時，它會從其上的管理群組階層繼承原則和角色指派。 當您為 Azure 工作負載建立多個訂用帳戶之後，您應該建立額外的訂用帳戶來包含其他訂用帳戶共用的 Azure 服務。
