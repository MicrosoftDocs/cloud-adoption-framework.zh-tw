...
ms.locfileid: "101787224"
---
# <a name="implement-cloud-adoption-framework-enterprise-scale-landing-zones-in-azure"></a>在 Azure 中實施雲端採用架構企業級登陸區域

當商務需求需要從一開始就完全整合的治理、安全性及營運，進行登陸區域的豐富初始實行時，請使用此處所列的企業規模範例選項。 透過此方法，您可以使用 Azure 入口網站或基礎結構即程式碼來設定和設定您的環境。 您也可以在您的組織準備好時，在入口網站與基礎結構即程式碼 (建議的) 之間進行轉換。
