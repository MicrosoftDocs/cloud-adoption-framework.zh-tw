---
title: 在 Azure 中執行雲端採用架構的企業級別登陸區域
description: 請參閱為 Azure 企業級架構實行雲端採用架構的選項。
...
ms.locfileid: "88040859"
---
# <a name="implement-cloud-adoption-framework-enterprise-scale-landing-zones-in-azure"></a>在 Azure 中執行雲端採用架構的企業級別登陸區域

當商務需求需要從一開始就具備完全整合的管理、安全性和作業來進行登陸區域的豐富初始執行時，請使用此處所列的企業規模範例選項。 使用此方法，您可以使用 Azure 入口網站或基礎結構即程式碼來設定您的環境。 當您的組織準備就緒時，您也可以在入口網站和基礎結構之間轉換為程式碼 (建議) 。
...
這些範例提供簡單的部署選項，以支援持續學習企業規模的方法。 在企業規模的生產版本中使用這些範例之前，請先參閱[企業規模架構](./architecture.md)。
