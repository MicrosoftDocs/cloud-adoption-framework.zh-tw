...
- 確定只有特殊許可權的使用者可以在管理群組階層 [設定](/azure/governance/management-groups/how-to/protect-resource-hierarchy#setting---require-authorization) 中啟用 Azure RBAC 授權，以在租使用者中操作管理群組 (預設會授權所有使用者在根管理群組) 下建立自己的管理群組。
- 為新的訂用帳戶[設定](/azure/governance/management-groups/how-to/protect-resource-hierarchy#setting---default-management-group)預設的專用管理群組，以確保不會在根管理群組下放置任何訂用帳戶。 如果有適合 MSDN 或 Visual Studio 權益與訂用帳戶的使用者，這就特別重要。 這類管理群組的理想選擇是 `Sandbox` 管理群組。
