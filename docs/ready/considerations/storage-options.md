...
| [Azure 磁碟儲存體](https://docs.microsoft.com/azure/virtual-machines/windows/managed-disks-overview) | Azure 磁碟儲存體提供持續性、高效能的區塊儲存體，以支援 Azure 虛擬機器。 Azure 磁片具有高耐用性、安全性，並為使用[premium 或 Ultra ssd](https://docs.microsoft.com/azure/virtual-machines/windows/disks-types)的 vm 提供業界唯一的單一實例 SLA。 Azure 磁碟可透過對應至 Azure 虛擬機器容錯網域的可用性設定組和可用性區域，提供高可用性。 此外，Azure 磁碟會作為 Azure 中的最上層資源來管理。 根據預設，系統會提供角色型存取控制 (RBAC)、原則和標記等 Azure Resource Manager 功能。 |
