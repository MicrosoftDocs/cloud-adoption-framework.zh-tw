...
ms.locfileid: "70819773"
---
# <a name="ready-recommended-naming-and-tagging-conventions"></a>準備就緒：建議的命名和標記慣例
...
Azure 架構中心的 [Azure 資源命名慣例](/azure/architecture/best-practices/naming-conventions)指南會提供命名慣例的一般建議，並討論命名限制和平台規則。 下列會討論支援企業雲端採用工作的一般指引，並加入詳細建議。
...
所有 Azure 資源類型都有一個用來定義如何管理這些資產的範圍 (相對於其他資源類型)。 就命名慣例而言，這表示資源在其範圍內必須有唯一的名稱。
...
資源名稱有長度限制。 當您開發命名慣例時，透過其範圍和長度來平衡名稱中的內嵌內容是很重要的。 若要深入了解命名規則中各資源類型允許的字元、範圍和名稱長度，請參閱 [Azure 資源的命名慣例](/azure/architecture/best-practices/naming-conventions)。
...
| 訂閱類型 | 資源所在訂用帳戶的用途摘要描述。 通常會依部署環境類型或特定工作負載來細分。 | *prod,* s*hared, client* |
...
| 嚴重損壞修復         | 應用程式、工作負載或服務的業務關鍵性。                                                                                                                                                | *DR*              | *Mission-critical, Critical, Essential*         |
| 專案的結束日期   | 排定淘汰應用程式、工作負載或服務的日期。                                                                                                                                  | *EndDate*         | *{date}*                                        |
...
| 專案的開始日期 | 初次部署應用程式、工作負載或服務的日期。                                                                                                                                           | *StartDate*       | *{date}*                                        |
...
| 資產類型     | `Scope`        | 格式                                                     | 範例                                                                            |
|----------------|--------------|------------------------------------------------------------|-------------------------------------------------------------------------------------|
| 資源群組 | 訂閱 | rg-\<應用程式 / 服務名稱\>-\<訂用帳戶類型\>-\<\#\#\#\> | <ul><li>rg-mktgsharepoint-prod-001 </li><li>rg-acctlookupsvc-share-001 </li><li>rg-ad-dir-services-shared-001</li></ul> |
...
| Subnet                   | 虛擬網路 | snet-\<subscription\>-\<subregion\>-\<\#\#\#\>                       | <ul><li>snet-shared-eastus2-001 </li><li>snet-prod-westus-001 </li><li>snet-client-eastus2-001</li></ul>                                  |
...
| 資產類型     | `Scope`  | 格式                                                              | 範例                                                                                 |
|----------------|--------|---------------------------------------------------------------------|------------------------------------------------------------------------------------------|
| Azure App Service    | 全域 | azapp-\<應用程式名稱\>-\<環境\>-\<\#\#\#\>.[{azurewebsites.net}] | <ul><li>azapp-navigator-prod-001.azurewebsites.net </li><li>azapp-accountlookup-dev-001.azurewebsites.net</li></ul> |
| Azure Functions 應用程式   | 全域 | azfun-\<應用程式名稱\>-\<環境\>-\<\#\#\#\>.[{azurewebsites.net}] | <ul><li>azfun-navigator-prod-001.azurewebsites.net </li><li>azfun-accountlookup-dev-001.azurewebsites.net</li></ul> |
| Azure 雲端服務 | 全域 | azcs-\<應用程式名稱\>-\<環境\>-\<\#\#\#\>.[{cloudapp.net}]       | <ul><li>azcs-navigator-prod-001.azurewebsites.net </li><li>azcs-accountlookup-dev-001.azurewebsites.net</li></ul>   |
...
| 資產類型                          | `Scope`              | 格式                                | 範例                                       |
|-------------------------------------|--------------------|---------------------------------------|------------------------------------------------|
| Azure SQL Database                  | 全域             | sqldb-\<應用程式名稱\>-\<環境\>    | <ul><li>sqldb-navigator-prod </li><li>sqldb-emissions-dev</li></ul>       |
| Azure Cosmos DB (先前稱為 DocumentDB) | 全域             | cosdb-\<應用程式名稱\>-\<環境\>    | <ul><li>cosdb-navigator-prod </li><li>cosdb-emissions-dev</li></ul>       |
| Azure Cache for Redis               | 全域             | redis-\<應用程式名稱\>-\<環境\>    | <ul><li>redis-navigator-prod </li><li>redis-emissions-dev</li></ul>       |
| 適用於 MySQL 的 Azure 資料庫            | 全域             | mysql-\<應用程式名稱\>-\<環境\>    | <ul><li>mysql-navigator-prod </li><li>mysql-emissions-dev</li></ul>       |
...
| Azure 認知服務               | 資源群組 | cs-\<應用程式名稱\>-\<環境\>   | <ul><li>cs-navigator-prod </li><li>cs-emissions-dev</li></ul>     |
| Azure Machine Learning 工作區 | 資源群組 | aml-\<應用程式名稱\>-\<環境\>  | <ul><li>aml-navigator-prod </li><li>aml-emissions-dev</li></ul>   |

### <a name="analytics"></a>分析

| 資產類型                | `Scope`  | 格式                             | 範例                                 |
|---------------------------|--------|------------------------------------|------------------------------------------|
| Azure Data Factory        | 全域 | df-\<應用程式名稱\>\<環境\>     | <ul><li>df-navigator-prod </li><li>df-emissions-dev</li></ul>       |
| Azure Data Lake Storage   | 全域 | dls\<應用程式名稱\>\<環境\>     | <ul><li>dlsnavigatorprod </li><li>dlsemissionsdev</li></ul>         |
| Azure Data Lake Analytics | 全域 | dla\<應用程式名稱\>\<環境\>     | <ul><li>dlanavigatorprod </li><li>dlaemissionsdev</li></ul>         |
| Azure HDInsight - Spark         | 全域 | hdis-\<應用程式名稱\>-\<環境\>  | <ul><li>hdis-navigator-prod </li><li>hdis-emissions-dev </li></ul>  |
| Azure HDInsight - Hadoop        | 全域 | hdihd-\<應用程式名稱\>-\<環境\> | <ul><li>hdihd-hadoop-prod </li><li>hdihd-emissions-dev</li></ul>    |
| Azure HDInsight - R Server      | 全域 | hdir-\<應用程式名稱\>-\<環境\>  | <ul><li>hdir-navigator-prod </li><li>hdir-emissions-dev</li></ul>   |
| Azure HDInsight - HBase         | 全域 | hdihb-\<應用程式名稱\>-\<環境\> | <ul><li>hdihb-navigator-prod </li><li>hdihb-emissions-dev</li></ul> |
| Power BI Embedded         | 全域 | pbiemb\<應用程式名稱\>\<環境\>  | <ul><li>pbiem-navigator-prod </li><li>pbiem-emissions-dev</li></ul> |

### <a name="internet-of-things-iot"></a>物聯網 (IoT)

| 資產類型                         | `Scope`          | 格式                             | 範例                                 |
|------------------------------------|----------------|------------------------------------|------------------------------------------|
| Azure IoT Edge 串流分析 | 資源群組 | asa-\<應用程式名稱\>-\<環境\>   | <ul><li>asa-navigator-prod </li><li>asa-emissions-dev</li></ul>     |
| Azure IoT 中樞                      | 全域         | aih-\<應用程式名稱\>-\<環境\>   | <ul><li>aih-navigator-prod </li><li>aih-emissions-dev</li></ul>     |
| Azure 事件中樞                          | 全域         | evh-\<應用程式名稱\>-\<環境\>   | <ul><li>evh-navigator-prod </li><li>evh-emissions-dev</li></ul>     |
| Azure 通知中樞                   | 資源群組 | anh-\<應用程式名稱\>-\<環境\>   | <ul><li>evh-navigator-prod </li><li>evh-emissions-dev</li></ul>     |
| Azure 通知中樞命名空間         | 全域         | anhns-\<應用程式名稱\>-\<環境\> | <ul><li>anhns-navigator-prod </li><li>anhns-emissions-dev</li></ul> |
