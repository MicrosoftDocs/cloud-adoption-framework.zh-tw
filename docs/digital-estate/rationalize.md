...
合理化的傳統方法會嘗試符合所有可預見的需求。 所幸，要開始進行一個轉型程序，通常不需要為每個應用程式設定一個計劃。 在累加式模型中，10 的乘冪是個很好的起點。 在此模型中，雲端策略小組會選取要遷移的前10個應用程式。 這十個工作負載應會同時包含簡單和複雜的工作負載。
