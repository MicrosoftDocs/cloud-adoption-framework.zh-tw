...
當您將合理化的傳統程式視覺化為複雜的決策樹時，很容易就能瞭解合理化。 數位資產中的每個資產都是透過一個程式來進行，這會產生五個答案的其中一個（這是五大的合理化）。 此程序非常適用於小型資產。 對於較大的資產，它沒有效率，而且可能會導致明顯的延遲。 我們將檢查該程序以了解原因。 然後，我們會產生更有效率的模型。

**清查：** 若要使用傳統模型完成完整的合理化，必須全面清查資產，包括應用程式、軟體、硬體、作業系統和系統效能計量。

**量化分析：** 在決策樹中，量化問題會推動第一層決策。 常見的問題包括下列各項：

    - 資產目前是否使用中？
    - 若是如此，它是否已最佳化並適當調整大小？
    - 資產之間有哪些相依性？ 這些問題對清查的分類很重要。
...
少陣列織會投入時間、能源和費用，以維護完整數位資產的精確即時清查。 遺失、遭竊、重新整理週期和員工上線通常會證明終端使用者裝置的詳細資產追蹤。 在傳統的內部部署資料中心內維護精確伺服器和應用程式清查的 ROI 通常較低。 與追蹤資料中心內的固定資產使用量相比，大部分的 IT 組織都有更緊急的問題需要解決。
...
在累加式合理化程式中，無代理程式解決方案可用於初始探索，以加速早期決策。 視環境中的複雜性層級而定，可能還是需要以代理程式為基礎的解決方案，但可以從商務變更的關鍵路徑中移除。
...
這個精簡方法會產生基準、財務計劃、策略和方向。 在後續的活動中，每個資產都會經過進一步的合理化和定性分析，以評估其他選項。 您在此初始合理化中進行的所有假設都是在遷移個別工作負載之前進行測試。
...
當您移至雲端帳戶處理模型時，淘汰資產可能會大幅節省年度營運成本和預先遷移的工作。
...
- 要求資料和 R&D 小組識別正在為新收益串流供電的資產，並將其從核心遷移計畫中移除。
...
雲端採用小組和雲端策略小組可以共同合作來分析此小型工作負載。 此共同作業會建立受控制的機會，以建立及測試定性的分析準則。 較小的人口會建立一個機會來調查受影響的使用者，並在一周或更短的時間內完成詳細的定性分析。 如需常見的定性分析因素，請參閱[五大合理化](./5-rs-of-rationalization.md)中的特定合理化目標。
...
合理化的傳統方法會嘗試符合所有可預見的需求。 所幸，要開始進行一個轉型程序，通常不需要為每個應用程式設定一個計劃。 在累加式模型中，10種方法的強大功能可提供良好的起點。 在此模型中，雲端策略小組會選取要遷移的前10個應用程式。 這十個工作負載應會同時包含簡單和複雜的工作負載。
...
當評量移至遷移的增量執行程式之後，雲端採用小組可以更快速地反復評估和架構。 在此階段中，雲端策略小組也會一併抽象化，減少其時間的耗盡。 這也可讓雲端策略小組專注于不在特定版本中的應用程式優先順序，以確保與不斷變化的市場狀況緊密一致。
...
經過一段時間，雲端採用小組和雲端策略小組會一起完成清查的完整合理化。 這種累加式方法可讓小組在合理化程式中持續更快地進行。 它也可協助轉換旅程更快地產生具體的商業結果，而不需要事先進行分析。
